...
{
  EGifOpenFileName  = (EGifOpenFileName_t)
                      GetProcAddress(dll, "EGifOpenFileName");
  MakeMapObject     = (MakeMapObject_t)
                      GetProcAddress(dll, "MakeMapObject");
  EGifSetGifVersion = (EGifSetGifVersion_t)
                      GetProcAddress(dll, "EGifSetGifVersion");
  EGifPutScreenDesc = (EGifPutScreenDesc_t)
                      GetProcAddress(dll, "EGifPutScreenDesc");
  EGifPutImageDesc  = (EGifPutImageDesc_t)
                      GetProcAddress(dll, "EGifPutImageDesc");
  EGifCloseFile     = (EGifCloseFile_t)
                      GetProcAddress(dll, "EGifCloseFile");
  FreeMapObject     = (FreeMapObject_t)
                      GetProcAddress(dll, "FreeMapObject");
  EGifPutLine       = (EGifPutLine_t)
                      GetProcAddress(dll, "EGifPutLine");

  if (!EGifOpenFileName || !MakeMapObject || !EGifSetGifVersion || !EGifPutScreenDesc ||
      !EGifPutImageDesc || !EGifCloseFile || !FreeMapObject || !EGifPutLine)
    return -1;
