      ...
      case WM_COMMAND:
        switch (LOWORD(wparam)) {
          ...
          case IDC_SELECT_DRIVE_TYPE_1571:
            ...
            break;
          case IDC_SELECT_DRIVE_TYPE_1571cr:
            resources_set_int_sprintf("Drive%dType", DRIVE_TYPE_1571CR, num);
            enable_controls_for_drive_settings(hwnd, LOWORD(wparam));
            break;
          case IDC_SELECT_DRIVE_TYPE_1581:
...
void uidrivec128_settings_dialog(HWND hwnd)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        psp[i].pszTemplate = MAKEINTRESOURCE(translate_res(IDD_DRIVE_SETTINGS_DIALOG_C128));
        psp[i].pszIcon = NULL;
#else
        psp[i].DUMMYUNIONNAME.pszTemplate = MAKEINTRESOURCE(translate_res(IDD_DRIVE_SETTINGS_DIALOG_C128));
