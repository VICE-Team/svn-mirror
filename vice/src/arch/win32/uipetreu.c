/*
 * uipetreu.c - Implementation of the PET RAM and Expansion Unit settings dialog box.
 *
 * Written by
...
static void enable_petreu_controls(HWND hwnd)
{
  ...
  is_enabled = (IsDlgButtonChecked(hwnd, IDC_PETREU_ENABLE)
               == BST_CHECKED) ? 1 : 0;
...
static void init_petreu_dialog(HWND hwnd)
{
  ...
  resources_get_value("PETREU", (void *)&res_value);
  ...
  resources_get_value("PETREUsize", (void *)&res_value);
  active_value = 0;
  for (res_value_loop = 0; res_value_loop < NUM_OF_PETREU_SIZE;
      res_value_loop++)
  {
  ...
  resources_get_value("PETREUfilename", (void *)&petreufile);
...
static void end_petreu_dialog(HWND hwnd)
{
  ...
  resources_set_value("PETREU", (resource_value_t)
                      (IsDlgButtonChecked
                      (hwnd, IDC_PETREU_ENABLE) == BST_CHECKED ?
                      1 : 0 ));
  resources_set_value("PETREUsize",(resource_value_t)
                      ui_petreu_size[SendMessage(GetDlgItem(
                      hwnd, IDC_PETREU_SIZE), CB_GETCURSEL, 0, 0)]);

  GetDlgItemText(hwnd, IDC_PETREU_FILE, st, MAX_PATH);
  system_wcstombs(s, st, MAX_PATH);
  resources_set_value("PETREUfilename", (resource_value_t)s);
}
