/*
 * joy-hid.c - Mac OS X joystick support using HID Utility Library.
 *
 * Written by
...
static int get_device_serial(pRecDevice last_device)
{
    int count = 0;
    pRecDevice device;
    ...
    *result = devs;
    ...
    for (device = HIDGetFirstDevice(); 
        ...
        if ((device->usage == kHIDUsage_GD_Joystick) || 
            ...
            devs->serial = get_device_serial(device);
    ...
    for (i = 0; i < joy->num_hid_axis; i++) {
        ...
        if (HIDIsValidElement(device, element)) {
            ...
            if((value < min) || (value > max)) {
                return find_axis_name(element->usage);
            }
        }
    }
    return NULL;    
