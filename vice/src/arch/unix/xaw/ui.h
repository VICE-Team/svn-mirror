...
typedef enum { UI_JAM_RESET, UI_JAM_HARD_RESET,
               UI_JAM_MONITOR, UI_JAM_DEBUG } UiJamAction;
...
extern void UiDisplaySpeed(float percent, float framerate, int warp_flag);
...
extern UiJamAction UiJamDialog(const char *format,...);
extern void UiMessage(const char *format,...);
extern void UiShowText(const char *title, const char *text, int width, int height);
extern char *UiFileSelect(const char *title, char *(*read_contents_func)(const char *), int allow_autostart, UiButton *button_return);
extern UiButton UiInputString(const char *title, const char *prompt, char *buf, unsigned int buflen);
