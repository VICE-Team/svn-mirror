...
if test x"$5" = "xzip"; then
  gcc $6/src/arch/unix/qnx/getsize.c -o ./getsize
  gcc $6/src/arch/unix/qnx/getlibs.c -o ./getlibs
  mkdir -p public/VICE/core-$2/x86/opt/bin
  mv VICE-$2/opt/bin/x* public/VICE/core-$2/x86/opt/bin
  mv VICE-$2/opt/bin/c1541 public/VICE/core-$2/x86/opt/bin
  mv VICE-$2/opt/bin/cartconv public/VICE/core-$2/x86/opt/bin
  mv VICE-$2/opt/bin/petcat public/VICE/core-$2/x86/opt/bin
   ...
   <RDF:Description about="http://www.qnx.com">
      <QPM:PackageManifest>
         <QPM:PackageDescription>
            ...
            <QPM:PackageName>VICE-$2-x86-public</QPM:PackageName>
_END
...
_END
  ...
  ls -l -R public/VICE/core-$2/x86/opt >size.tmp
...
echo >manifest.07 "            <QPM:InstallPath>public/VICE/core-$2/x86</QPM:InstallPath>"
...
cat >manifest.08 <<_END
         ...
         <QPM:ReleaseDescription>
            ...
            <QPM:ReleaseVersion>$2</QPM:ReleaseVersion>
         ...
         <QPM:LicenseUrl>rep://VICE-$2-public.repdata/LicenseUrl/COPYING</QPM:LicenseUrl>
         <QPM:ProductKeyUrl/>
...
_END
  ...
  ./getlibs public/VICE/core-$2/x86/opt/bin/x64 >manifest.13
...
_END
  ...
  cat >public/VICE/core-$2/x86/MANIFEST manifest.01 manifest.02 \
  ...
  gzip VICE-x86.tar
  mv VICE-x86.tar.gz VICE-$2-x86-public.qpk

  ls -l -R VICE-$2-x86-public.qpk >size.tmp
  manifest_size=`./getsize size.tmp` 
  rm -f -r size.tmp

  echo >manifest.04 "            <QPM:PackageSize>$manifest_size</QPM:PackageSize>"

  cat >VICE-$2-x86-public.qpm manifest.01 manifest.02 manifest.03 manifest.04 \
  ...
  mkdir -p public/VICE/core-$2
  mv VICE-$2/opt public/VICE/core-$2

  mkdir -p VICE-$2-public.repdata/LicenseUrl
  cp public/VICE/core-$2/opt/lib/vice/doc/COPYING VICE-$2-public.repdata/LicenseUrl

  echo >manifest.02 "            <QPM:PackageSize/>"

  ls -l -R public/VICE/core-$2/opt >size.tmp
  manifest_size=`./getsize size.tmp`
  rm -f -r size.tmp
  echo >manifest.04 "            <QPM:ProductSize>$manifest_size</QPM:ProductSize>"
  ...
  echo >manifest.07 "            <QPM:InstallPath>public/VICE/core-$2</QPM:InstallPath>"
...
_END
  ...
  cat >public/VICE/core-$2/MANIFEST manifest.01 manifest.02 manifest.03 \
  ...
  gzip VICE-core.tar
  mv VICE-core.tar.gz VICE-$2-public.qpk

  ls -l -R VICE-$2-public.qpk >size.tmp
  manifest_size=`./getsize size.tmp` 
  rm -f -r size.tmp

  echo >manifest.04 "            <QPM:PackageSize>$manifest_size</QPM:PackageSize>"

  cat >VICE-$2-public.qpm manifest.01 manifest.02 manifest.03 manifest.04 \
      manifest.05 manifest.06 manifest.07 manifest.08 manifest.09 manifest.10 \
      manifest.11 manifest.12 manifest.14 manifest.15

  rm -f -r public

  tar cf VICE-x86.tar VICE-$2-public.qp? VICE-$2-public.repdata VICE-$2-x86-public.qp?
  gzip VICE-x86.tar
  mv VICE-x86.tar.gz VICE-$2-x86-public.qpr

  rm -f -r *.qpk *.qpm *.repdata manifest.* VICE-$2

  echo QNX port binary package part generated as VICE-$2-x86-public.qpr
else
