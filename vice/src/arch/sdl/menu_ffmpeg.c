...
UI_MENU_DEFINE_SLIDER(FFMPEGVideoBitrate, 100000, 10000000)
UI_MENU_DEFINE_SLIDER(FFMPEGAudioBitrate, 16000, 128000)
...
static UI_MENU_CALLBACK(save_movie_callback)
{
    ...
    if (activated) {
        ...
        name = sdl_ui_file_selection_dialog("Choose movie file", FILEREQ_MODE_CHOOSE_FILE);
        if (name != NULL) {
