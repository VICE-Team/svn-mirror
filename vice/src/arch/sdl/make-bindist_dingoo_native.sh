...
for i in $EXECUTABLES
do
  cp src/$i Dingoo-$VICEVERSION/$i.elf
  $DINGOO_SDK/tools/elf2app/elf2app Dingoo-$VICEVERSION/$i
  rm Dingoo-$VICEVERSION/$i.elf
  rm Dingoo-$VICEVERSION/$i.bin
done

cp src/arch/sdl/dingoo-files/vicerc-native Dingoo-$VICEVERSION/vicerc
cp src/arch/sdl/dingoo-files/hotkey-native Dingoo-$VICEVERSION/sdl-hotkey-C64.vkm
