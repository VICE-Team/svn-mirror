...
static GtkWidget *create_acia_widget(void)
{
    ...
    /* Acia1Dev */
    label = create_indented_label("ACIA device");
    ...
    if (acia_base_widget != NULL) {
        label = create_indented_label("ACIA base address");
        gtk_grid_attach(GTK_GRID(grid), label, 0, row, 1, 1);
    ...
    if (acia_irq_widget != NULL) {
        label = create_indented_label("ACIA IRQ");
        gtk_grid_attach(GTK_GRID(grid), label, 0, row, 1, 1);
    ...
    if (acia_mode_widget != NULL) {
        label = create_indented_label("ACIA emulation mode");
        gtk_grid_attach(GTK_GRID(grid), label, 0, row, 1, 1);
