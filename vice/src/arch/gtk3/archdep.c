        ...
        void (*cleaner)(pcap_if_t *))
{
    ...
    /* get list of devices */
    if (getter(&devices, errbuf) != 0) {
        log_error(LOG_ERR, "pcap_findalldevs() failed: %s\n", errbuf);
        return NULL;
    }

    /* get number of devices in list that are UP */
    for (dev = devices; dev != NULL; dev = dev->next) {
        if (dev->flags & PCAP_IF_UP) {
            num++;
        }
    }
    /* any empty list is not considered an error by libpcap */
    if (num == 0) {
        log_error(LOG_ERR, "pcap_findalldevs() returned an empty list\n");
        return NULL;
    }
...
/** \brief  Free the list of devices generated by archdep_get_net_devices()
 *
