...
typedef struct vice_opengl_renderer_context_s {
    /** \brief Used to smooth out asynchronous rendering and implement vsync */
    void *render_queue;
...
static gboolean render_opengl_cb (GtkGLArea *area, GdkGLContext *unused, gpointer data)
{
    ...
    /* Attempt to obtain a new frame to display */
    ...
    if (ctx->render_mode == RENDER_MODE_NEW_TEXTURE) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ctx->true_width, ctx->true_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, backbuffer);
        ...
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glPixelStorei(GL_UNPACK_ROW_LENGTH, ctx->true_width);
        glTexSubImage2D(GL_TEXTURE_2D, 0, ctx->dirty_x, ctx->dirty_y, ctx->dirty_w, ctx->dirty_h, GL_RGBA, GL_UNSIGNED_BYTE, backbuffer + 4 * (ctx->true_width * ctx->dirty_y + ctx->dirty_x));
        glBindTexture(GL_TEXTURE_2D, 0);
    }
    ...
    /* Return the backbuffer to be used again */
    // render_queue_return_to_pool(ctx->render_queue, backbuffer);

    // Without this, all we see is the glClearColor on macOS
    glFlush();
...
static void vice_opengl_destroy_context(video_canvas_t *canvas)
{
    if (canvas) {
        ...
        /* TODO: delete textures, shaders, etc */
                                     ...
                                     unsigned int w, unsigned int h)
{
    ...
    video_canvas_render(canvas, backbuffer, w, h, xs, ys, xi, yi, ctx->true_width * 4, 32);

    /* Place the rendered buffer in the render queue */
    // render_queue_enqueue_for_display(ctx->render_queue, backbuffer);

    /* Update the dirty vars. TODO: Should we simplify this by removing it --dqh */
...
static void vice_opengl_queue_redraw(GdkFrameClock *clock, video_canvas_t *canvas)
{
    context_t *ctx = canvas ? (context_t *)canvas->renderer_context : NULL;
