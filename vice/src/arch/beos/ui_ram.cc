		...
		B_NOT_ZOOMABLE | B_NOT_RESIZABLE) 
{
	...
	for (i=0; ui_ram_startvalue[i] >= 0; i++) {
		...
		radiobutton = new BRadioButton(BRect(10+50*i,15,40+50*i,35),
			str, str, msg);
	...
	valueinvertlistview = new BListView(BRect(5, 20, 40, 180), 
		"Value Invert");
	valueinvertlistview->SetSelectionMessage(
		new BMessage(MESSAGE_RAM_VALUEINVERT));
	box->AddChild(valueinvertlistview);

	patterninvertlistview = new BListView(BRect(55, 20, 95, 180), 
		"Pattern Invert");
	patterninvertlistview->SetSelectionMessage(
		new BMessage(MESSAGE_RAM_PATTERNINVERT));
	...
	for (i=0; ui_ram_invertvalue[i]>=0; i++) {
        ...
        if (ui_ram_invertvalue[i] == orig_valueinvert)
			valueinvertlistview->Select(
				valueinvertlistview->IndexOf(item));
		patterninvertlistview->AddItem(item = new BStringItem(str));
        if (ui_ram_invertvalue[i] == orig_patterninvert)
			patterninvertlistview->Select(
				patterninvertlistview->IndexOf(item));
	}
