...
static char* cbm2_model_name[] = {
	"610", "620", "620+", "710", "720", "720+", NULL
};

static char *cbm2_modelline[] = {
	"7x0 (50Hz)", "6x0 60Hz", "6x0 50Hz", NULL
};

static int cbm2_memory[] = { 128, 256, 512, 1024, 0 };

static char *rambank_res[] = {
	"Ram08", "Ram1", "Ram2", "Ram4", "Ram6", "RamC", NULL
};
		...
		B_NOT_ZOOMABLE | B_NOT_RESIZABLE) 
{
	...
	for (i=0; cbm2_model_name[i]; i++)
	{
		msg = new BMessage(MESSAGE_CBM2_MODEL);
		msg->AddInt32("model", i);
		button = new BButton(BRect(10, 15+i*25, 90, 30+i*25),
			cbm2_model_name[i],
			cbm2_model_name[i],
			msg);
		box->AddChild(button);
	...
	for (i=0; cbm2_modelline[i]; i++)
	{
		...
		radiobutton = new BRadioButton(BRect(10, 15+i*25, 90, 25+i*25),
			cbm2_modelline[i],
			cbm2_modelline[i],
			msg);
	...
	for (i=0; cbm2_memory[i]; i++)
	{
		...
		radiobutton = new BRadioButton(BRect(10, 15+i*25, 150, 25+i*25),
			str,
			str,
			msg);
	...
	for (i=0; rambank_text[i]; i++)
	{
		...
		checkbox = new BCheckBox(BRect(10, 15+i*25, 150, 25+i*25),
			rambank_text[i],
			rambank_text[i],
			msg);
...
void Cbm2Window::MessageReceived(BMessage *msg) {
	...
	switch (msg->what) {
		...
		case MESSAGE_CBM2_MODEL:
			msg->FindInt32("model", &res_value);
			cbm2_set_model(cbm2_model_name[res_value], NULL);
			break;
