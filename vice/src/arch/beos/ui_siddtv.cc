    ...
    : BWindow(BRect(250, 50, 500, 250), "Sid settings", B_TITLED_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL, B_NOT_ZOOMABLE | B_NOT_RESIZABLE) 
{
    ...
    checkbox = new BCheckBox(BRect(10, 60, r.Width() / 2 - 40, 75), "SID Filters", "SID Filters", new BMessage(MESSAGE_SIDDTV_FILTERS));
    checkbox->SetValue(res_val);
    background->AddChild(checkbox);

    /* ReSID */
    resources_get_int("SidEngine", &res_val);
    checkbox = new BCheckBox(BRect(10, 90, r.Width() / 2 - 20, 105), "Enable reSID-DTV", "Enable reSID-DTV", new BMessage(MESSAGE_SIDDTV_RESID));
    ...
    passbandslider = new BSlider(BRect(r.Width() / 2 + 10, 20, r.Width() - 10, 60), "Passband", "Passband", new BMessage(MESSAGE_SIDDTV_RESIDPASSBAND), 0, 90, B_TRIANGLE_THUMB);
