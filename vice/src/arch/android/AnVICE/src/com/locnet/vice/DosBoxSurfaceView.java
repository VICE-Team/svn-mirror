...
public class DosBoxSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
		...
		public void handleMessage (Message msg) {
			switch (msg.what) {
				case DosBoxLauncher.SPLASH_TIMEOUT_MESSAGE:
					...
					//DosBoxMenuUtility.doSendDownUpKey(mParent, DosBoxMenuUtility.KEYCODE_NUM_LOCK);				
					break;
		...
		try {
			...
			{				
				...
				if (mScale) {
					...
					if ((isLandscape)
						&& (mParent.mPrefScaleMode == SCALE_MODE_FIT_WIDTH)
				...
				if ((mShowInfo || mParent.mPrefAlwaysShowJoystick) && (mInputMode == INPUT_MODE_JOYSTICK)) {
					//drawButton(canvas, 0, screen_height/ONSCREEN_BUTTON_HEIGHT, screen_width, screen_height*(ONSCREEN_BUTTON_HEIGHT-1)/ONSCREEN_BUTTON_HEIGHT, "[+]");
	...
	void setJoystickRect() {
		...
		if (isLandscape(this)) {
			//mJoystickRect.set(0, screen_height/ONSCREEN_BUTTON_HEIGHT, height, screen_height*(ONSCREEN_BUTTON_HEIGHT-1)/ONSCREEN_BUTTON_HEIGHT);
	...
	private void handleJoystickMove(float x, float y) {
		//float x = (event.getX() / getWidth())* 2 - 1.0f;
		//float y = event.getY();
		//int min_y = getHeight()/ONSCREEN_BUTTON_HEIGHT; 
		//int max_y = getHeight()*(ONSCREEN_BUTTON_HEIGHT-1)/ONSCREEN_BUTTON_HEIGHT;
		//setJoystickRect();
	...
	/*
	@Override
	public boolean onKeyLongPress(int keyCode, final KeyEvent event) {
	...
	private boolean handleKey(int keyCode, final KeyEvent event) {
		...
		case KeyEvent.KEYCODE_VOLUME_DOWN:
			if (mParent.mPrefHardkeyOn) {
				...
				case INPUT_MODE_PEN:
					...
					//mAbsoluteMouseButton = (event.getAction() == KeyEvent.ACTION_DOWN)?1:0;
					if (event.getAction() == KeyEvent.ACTION_DOWN) {
		...
		default:
			...
			if (!down || (event.getRepeatCount() == 0)) {
				...
				//Log.d("c64", "down:" + down + ",isAlt:" + event.isAltPressed() + ",isShift:" + event.isShiftPressed() + ",repeat:" + event.getRepeatCount() + ",unicode:" + event.getUnicodeChar() + ",keycode:" + keyCode + ",flags:"+event.getFlags());
				...
				switch (keyCode) {
					...
					case DosBoxMenuUtility.KEYCODE_BUTTON_A:
						//unicode = mParent.mPrefKeyMapping.charAt(0);
