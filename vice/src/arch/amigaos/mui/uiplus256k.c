...
static video_canvas_t *plus256k_canvas;
...
static ui_to_from_t ui_to_from[] = {
    { NULL, MUI_TYPE_CYCLE, "PLUS256K", ui_plus256k_enable, ui_plus256k_enable_values, NULL },
...
static ULONG Browse(struct Hook *hook, Object *obj, APTR arg)
{
    ...
    fname = BrowseFile(translate_text(IDS_PLUS256K_FILENAME_SELECT), "#?", plus256k_canvas);
...
static APTR build_gui(void)
{
    ...
    ui = GroupObject,
           ...
           FILENAME(ui_to_from[1].object, translate_text(IDS_PLUS256K_FILENAME), browse_button)
           OK_CANCEL_BUTTON
         End;
...
void ui_plus256k_settings_dialog(video_canvas_t *canvas)
{
    APTR window;

    plus256k_canvas = canvas;
    intl_convert_mui_table(ui_plus256k_enable_translate, ui_plus256k_enable);

    window = mui_make_simple_window(build_gui(), translate_text(IDS_PLUS256K_SETTINGS));
