...
static video_canvas_t *plus60k_canvas;
...
static ULONG Browse( struct Hook *hook, Object *obj, APTR arg )
{
  ...
  fname=BrowseFile(translate_text(IDS_PLUS60K_FILENAME_SELECT), "#?", plus60k_canvas);
...
static APTR build_gui(void)
{
  ...
  ui = GroupObject,
    ...
    FILENAME(ui_to_from[2].object, translate_text(IDS_PLUS60K_FILENAME), browse_button)
    OK_CANCEL_BUTTON
  End;
...
void ui_plus60k_settings_dialog(video_canvas_t *canvas)
{
  APTR window;

  plus60k_canvas=canvas;
  intl_convert_mui_table(ui_plus60k_enable_translate, ui_plus60k_enable);

  window = mui_make_simple_window(build_gui(), translate_text(IDS_PLUS60K_SETTINGS));
