...
int ui_load_template(const char *tempname, RO_Window **wptr, wimp_msg_desc *msg)
{
    if ((*wptr = wimp_load_template(tempname)) == NULL) {
        ...
        if (wptr == &EmuWindow) {
            ...
            w->vminx = (ScreenMode.resx - dx) / 2;
            w->vmaxx = w->vminx + dx;
            w->vminy = (ScreenMode.resy - dy) / 2;
            w->vmaxy = w->vminy + dy;
            w->wmaxx = dx;
            w->wminy = -dy;
            title = EmuTitle;
        ...
        if (msg != NULL) {
            wimp_message_translate_window(msg, w);
        }
...
static void ui_display_sound_enable(int state)
{
    ...
    if ((machine_class == VICE_MACHINE_C64) || (machine_class == VICE_MACHINE_C64SC) || (machine_class == VICE_MACHINE_C128) || (machine_class == VICE_MACHINE_CBM5x0) || (machine_class == VICE_MACHINE_CBM6x0)) {
...
{
    if (resources_set_int(Rsrc_Sound, state) != 0) {
        return;
    }
...
static void ui_set_menu_display_text(const disp_desc_t *dd, int number, RO_MenuHead *menu)
{
    if (ConfWindows[dd->id.win] != NULL) {
        ...
        if ((icon = wimp_window_get_icon(ConfWindows[dd->id.win], dd->id.icon)) == NULL) {
            return;
        }
        if ((icon->iflags & IFlg_Indir) == 0) {
            return;
        }
...
static int ui_set_drive_image(int number, const char *file)
{
    if (!vsid_mode) {
        ...
        if ((ReadCatalogueInfo(file, info) & 1) == 0) {
            return -1;
        }
...
static int ui_set_drive_dir(int number, const char *dir)
{
    if (!vsid_mode) {
        ...
        if ((ReadCatalogueInfo(dir, info) & 2) == 0) {
            return -1;
        }
        file_system_detach_disk(8 + number);
...
static void ui_draw_sound_volume(int *b)
{
    ...
    /* Clip */
    if ((b[RedrawB_CMinX] > maxx) || (b[RedrawB_CMaxX] < minx) || (b[RedrawB_CMinY] > maxy) || (b[RedrawB_CMaxY] < miny)) {
        return;
    }
...
const char *ui_check_for_syspath(const char *path)
{
    ...
    if ((vicepath = getenv(VicePathVariable)) == NULL) {
        return path;
    }
    len = strlen(vicepath);
...
static int ui_build_romset_menu(void)
{
    ...
    if ((MenuROMSet != NULL) && (MenuDisplayROMSet != NULL)) {
        ...
        for (i = 0; i < number; i++) {
            ...
            if ((name = romset_archive_get_item(i)) == NULL) {
                name = "";
            }
            item[i].mflags = 0;
            item[i].submenu = (RO_MenuHead*)-1;
            item[i].iflags = Menu_Flags;
...
static int ui_build_fliplist_menu(int doread)
{
    ...
    if ((MenuFlipImages != NULL) && (MenuFlipImgNames != NULL)) {
        ...
        firstitem = (RO_MenuItem*)(MenuFlipImages + 1);
        item = firstitem;
        item->mflags |= MFlg_Dotted;
        item->mflags &= ~MFlg_LastItem;
        item++;
        iter = fliplist_init_iterate(FlipListDrive + 8);
        while (iter != NULL) {
            const char *use;
            ...
            strcpy(b, use);
            item->mflags = 0;
            item->submenu = (RO_MenuHead*)-1;
            item->iflags = Menu_Flags | IFlg_Indir;
            item->dat.ind.tit = (int*)b;
            item->dat.ind.val = NULL;
            item->dat.ind.len = len;
            b += len;
            iter = fliplist_next_iterate(FlipListDrive + 8);
            item++;
        }
...
int ui_emulator_init_epilogue(wimp_msg_desc *msg)
{
    ...
    dat = &(item->dat.ind);
    dat->tit = (int*)NewRomSetName;
    dat->val = (int*)-1; dat->len = sizeof(NewRomSetName);
    NewRomSetName[0] = '\0';
    ...
    dat->tit = (int*)VideoSyncCustomField;
    dat->val = (int*)-1;
    dat->len = sizeof(VideoSyncCustomField);
...
static void ui_open_config_window(int wnum)
{
    ...
    if (w != NULL) {
        ...
        if ((block[WindowB_WFlags] & (1 << 16)) == 0) {
            ...
            block[WindowB_ScrollX] = 0;
            block[WindowB_ScrollY] = 0;

            ui_setup_config_window(wnum);

            if (wnum == CONF_WIN_JOY) {
                JoystickWindowOpen = 1;
            }
        }
...
void ui_open_emu_window(RO_Window *win, int *b)
{
    ...
    if (b == NULL) {
        ...
        aux[WindowB_VMinX] = (ScreenMode.resx - dx) / 2;
        aux[WindowB_VMaxX] = aux[WindowB_VMinX] + dx;
        aux[WindowB_VMinY] = (ScreenMode.resy - dy) / 2;
        aux[WindowB_VMaxY] = aux[WindowB_VMinY] + dy;
        aux[WindowB_ScrollX] = 0;
        aux[WindowB_ScrollY] = 0;
    ...
    if ((ShowEmuPane != 0) && (ActiveCanvas != NULL) && (win == ActiveCanvas->window)) {
        ...
        if (x < 0) {
            if (block[WindowB_VMinX] < 0) {
                x = block[WindowB_VMinX];
            } else {
                x = 0;
            }
        }
        ...
        for (i = 0; i < CONF_WIN_NUMBER; i++) {
            if (ConfWinPositions[i] != NULL) {
                if (ConfWinPositions[i]->Handle == b[WindowB_Handle]) {
                    if (i == CONF_WIN_JOY) {
                        JoystickWindowOpen = 0;
                    }
                    ConfWinPositions[i] = NULL;
                    break;
                }
            }
        }
...
static int ui_create_new_disc_image(void)
{
    ...
    switch(number) {
        ...
        case Menu_CrtDisc_D71:
            type = DISK_IMAGE_TYPE_D71;
            break;
        case Menu_CrtDisc_D81:
            type = DISK_IMAGE_TYPE_D81;
            break;
        case Menu_CrtDisc_D80:
            type = DISK_IMAGE_TYPE_D80;
            break;
        case Menu_CrtDisc_D82:
            type = DISK_IMAGE_TYPE_D82;
            break;
        default:
            return -1;
    }
    ...
    if (b[MouseB_Buttons] != 2) {
        switch (b[MouseB_Icon]) {
            ...
            case Icon_Pane_Reset:
                if (b[MouseB_Buttons] == 1) {
                    mem_powerup();	/* adjust ==> hard reset */
                }
                ui_issue_reset(1);
                break;
        ...
        if (((posx >= iblock[IconB_MinX]) && (posx <= iblock[IconB_MaxX])) && ((posy >= iblock[IconB_MinY]) && (posy <= iblock[IconB_MaxY]))) {
            RO_MenuHead *menu;
...
static void ui_drag_snapshot_trap(WORD unused_address, void *unused_data)
{
    ...
    if ((WithinUiPoll == 0) && (SoundSuspended != 0) && (EmuPaused == 0)) {
        sound_resume();
    }
...
static void ui_user_drag_box(int *b)
{
    ...
    switch (LastDrag) {
        case DRAG_TYPE_SOUND:
            iconnum = Icon_ConfSnd_FileSndPath;
            win = ConfWindows[CONF_WIN_SOUND];
            break;
        case DRAG_TYPE_SERIAL:
            iconnum = Icon_ConfDev_FileRsPath;
            win = ConfWindows[CONF_WIN_DEVICES];
            break;
        case DRAG_TYPE_PRINTER:
            iconnum = Icon_ConfDev_FilePrPath;
            win = ConfWindows[CONF_WIN_DEVICES];
            break;
        case DRAG_TYPE_SNAPSHOT:
            iconnum = Icon_Snap_Path;
            win = SnapshotWindow;
            break;
        case DRAG_TYPE_SAVEBOX:
            iconnum = Icon_Save_Path;
            win = SaveBox;
            switch (LastSubDrag) {
                case SBOX_TYPE_ROMSET:
                case SBOX_TYPE_KEYBOARD:
        ...
        case DRAG_TYPE_CREATEDISC:
            iconnum = Icon_Create_File;
            win = CreateDiscWindow;
            break;
        default:
            break;
    }
    ...
    if (win != NULL) {
        ...
        if ((block[MouseB_Window] != -1) && ((block[MouseB_Window] != -2) || (block[MouseB_Icon] != IBarIcon.IconHandle))) {
            ...
            if ((h != 0) && (canvas_for_handle(h) == NULL) && (h != EmuPane->Handle) && (h != SaveBox->Handle) &&
                (h != ImgContWindow->Handle) && (ui_message_window_for_handle(h) == msg_win_NUMBER)) {
                char *name;
                ...
                block[5] = block[MouseB_Window];
                block[6] = block[MouseB_Icon];
                block[7] = block[MouseB_PosX];
                block[8] = block[MouseB_PosY];
                block[9] = estsize;
                block[10] = filetype;
                wimp_strcpy(((char*)block) + 44, name);
                block[MsgB_Size] = 44 + ((wimp_strlen(name) + 4) & ~3);
                block[MsgB_YourRef] = 0;
                block[MsgB_Action] = Message_DataSave;
...
static int ui_poll_joystick_window(int icon)
{
    if (wimp_window_has_input_focus(ConfWindows[CONF_WIN_JOY]) != 0) {
        ...
        for (i = 0; i < 2; i++) {
            if (icon == JoyToIcon[i].up) {
                dest = &(JoystickKeys[i].up);
            } else if (icon == JoyToIcon[i].down) {
                dest = &(JoystickKeys[i].down);
            } else if (icon == JoyToIcon[i].left) {
                dest = &(JoystickKeys[i].left);
            } else if (icon == JoyToIcon[i].right) {
                dest = &(JoystickKeys[i].right);
            } else if (icon == JoyToIcon[i].fire) {
                dest = &(JoystickKeys[i].fire);
            }
            if (dest != NULL) {
                break;
            }
    ...
    for (wnum = 0; wnum < CONF_WIN_NUMBER; wnum++) {
        ...
        switch (wnum) {
            case CONF_WIN_DRIVES:
                switch (b[KeyPB_Icon]) {
                    case Icon_ConfDrv_DriveFile11:
                        i++;
                    case Icon_ConfDrv_DriveFile10:
                        i++;
                    case Icon_ConfDrv_DriveFile9:
                        i++;
            ...
            case CONF_WIN_SYSTEM:
                switch (b[KeyPB_Icon]) {
                    case Icon_ConfSys_PollEvery:
                        resources_set_int(Rsrc_Poll, atoi(data));
                        break;
                    case Icon_ConfSys_SpeedEvery:
                        resources_set_int(Rsrc_Speed, atoi(data));
                        break;
                    case Icon_ConfSys_SoundEvery:
                        resources_set_int(Rsrc_SndEvery, atoi(data));
                        break;
                    case Icon_ConfSys_CharGen:
                        resources_set_string(Rsrc_CharGen, data);
                        break;
                    case Icon_ConfSys_Kernal:
                        resources_set_string(Rsrc_Kernal, data);
                        break;
                    case Icon_ConfSys_Basic:
                        resources_set_string(Rsrc_Basic, data);
                        break;
                    case Icon_ConfSys_CartFile:
                        ui_set_cartridge_file(data);
                        break;
            ...
            case CONF_WIN_VIDEO:
                switch (b[KeyPB_Icon]) {
                    case Icon_ConfVid_MaxSkipFrms:
                        resources_set_int(Rsrc_MaxSkipped, atoi(data));
                        break;
                    case Icon_ConfVid_FullScrNorm:
                        resources_set_string(Rsrc_FullScrNorm, data);
                        break;
                    case Icon_ConfVid_FullScrPAL:
                        resources_set_string(Rsrc_FullScrPal, data);
                        break;
                    case Icon_ConfVid_FullScrDbl:
                        resources_set_string(Rsrc_FullScrPal2, data);
                        break;
                    case Icon_ConfVid_ColourSat:
                        resources_set_int(Rsrc_ColourSat, atoi(data));
                        break;
                    case Icon_ConfVid_Contrast:
                        resources_set_int(Rsrc_Contrast, atoi(data));
                        break;
                    case Icon_ConfVid_Brightness:
                        resources_set_int(Rsrc_Brightness, atoi(data));
                        break;
                    case Icon_ConfVid_Gamma:
                        resources_set_int(Rsrc_Gamma, atoi(data));
                        break;
                    case Icon_ConfVid_LineShade:
                        resources_set_int(Rsrc_LineShade, atoi(data));
                        break;
                    case Icon_ConfVid_Palette:
                        resources_set_string(Rsrc_Palette, data);
                        break;
                    default:
                        Wimp_ProcessKey(key);
                        return;
            ...
            case CONF_WIN_SOUND:
                switch (b[KeyPB_Icon]) {
                    ...
                    case Icon_ConfSnd_ResidPass:
                        resources_set_int(Rsrc_ReSidPass, atoi(data));
                        break;
                    default:
                        Wimp_ProcessKey(key);
                        return;
                ...
                break;
            default:
	          if ((ViceMachineCallbacks.key_pressed_config == NULL) || (ViceMachineCallbacks.key_pressed_config(b, wnum, data) != 0)) {
                    Wimp_ProcessKey(key);
                }
                return;
        }
...
static void ui_key_press(int *b)
{
    ...
    if (b[KeyPB_Window] == ConfWindows[CONF_WIN_JOY]->Handle) {
        ...
        if (ui_poll_joystick_window(mpos[MouseB_Icon]) != 0) {
            return;
        }
    }
    if (key == Key_Return) {
        ...
        case Menu_IBar_FullScreen:
            {
                if ((!vsid_mode) && (video_full_screen_on(SpriteArea) != 0)) {
                    ...
                    err.errnum = 0;
                    strcpy(err.errmess, SymbolStrings[Symbol_ErrFullScr]);
                    Wimp_ReportError(&err, 1, WimpTaskName);
                }
            }
            break;
...
static void ui_load_snapshot_trap(WORD unused_address, void *unused_data)
{
    ...
    if (status == 0) {
        ...
        err.errnum = 0;
        strcpy(err.errmess, SymbolStrings[Symbol_ErrSnapR]);
        Wimp_ReportError(&err, 1, WimpTaskName);
        ui_issue_reset(1);
    }
...
static void ui_user_msg_data_load(int *b)
{
    ...
    if (canvas != NULL) {
        ...
        ui_image_contents_generic(name, b[10]);
        action = 1;
    } else if (b[5] == ConfWindows[CONF_WIN_DRIVES]->Handle) {
        for (i = 0; i < 4; i++) {
            if (b[6] == DriveToFile[i]) {
                break;
            }
        }
        if (i < 4) {
        ...
        if ((b[10] == FileType_Data) || (b[10] == FileType_Text)) {
            ...
            if (b[10] == FileType_Data) {
                if (b[6] == Icon_ConfSys_CharGen) {
                    res = Rsrc_CharGen;
                } else if (b[6] == Icon_ConfSys_Kernal) {
                    res = Rsrc_Kernal;
                } else if (b[6] == Icon_ConfSys_Basic) {
                    res = Rsrc_Basic;
                }
                if (res != NULL) {
                    rom_changed = 1;
    ...
    if (action != 0) {
        b[MsgB_YourRef] = b[MsgB_MyRef];
        b[MsgB_Action] = Message_DataLoadAck;
...
static void ui_user_msg_data_save(int *b)
{
    ...
    if (canvas != NULL) {
        if (((b[10] == FileType_C64File) && ((machine_class == VICE_MACHINE_C64) || (machine_class == VICE_MACHINE_C64SC)) || (b[10] == FileType_Data)) {
            action = 1;
        }
    } else if (b[5] == ConfWindows[CONF_WIN_SYSTEM]->Handle) {
        if ((b[10] == FileType_Data) || (b[10] == FileType_Text)) {
    ...
    if (action != 0) {
        ...
        WimpScrapUsed = 1;
        b[MsgB_YourRef] = b[MsgB_MyRef];
        b[MsgB_Action] = Message_DataSaveAck;
        Wimp_SendMessage(17, b, b[MsgB_Sender], b[6]);
    }
}
...
static void ui_user_msg_data_save_ack(int *b)
{
    ...
    switch (LastDrag) {
        case DRAG_TYPE_SOUND:
            ui_set_sound_file(name);
            break;
        case DRAG_TYPE_SERIAL:
            ui_set_serial_file(name);
            break;
        case DRAG_TYPE_PRINTER:
            ui_set_printer_file(name);
            break;
        case DRAG_TYPE_SNAPSHOT:
            if (ui_make_snapshot(name) == 0) {
                ...
                wimp_strcpy(((char*)SnapshotMessage) + 44, name);
                b[MsgB_YourRef] = b[MsgB_MyRef];
                b[MsgB_Action] = Message_DataLoad;
...
static void ui_user_msg_help_request(int *b)
{
    ...
    if (msg != NULL) {
        ...
        b[0] = (20 + len + 4) & ~3;
        b[MsgB_YourRef] = b[MsgB_MyRef];
        b[MsgB_Action] = Message_HelpReply;
        Wimp_SendMessage(17, b, b[MsgB_Sender], b[6]);
    }
}
...
int ui_poll_core(int *block)
{
    ...
    switch (event) {
        ...
        case WimpEvt_RedrawWin:
            ...
            break;
        case WimpEvt_OpenWin:
            ui_open_window(block);
            break;
        case WimpEvt_CloseWin:
            ui_close_window(block);
            break;
        case WimpEvt_MouseClick:
            ui_mouse_click(block);
            break;
        case WimpEvt_UserDragBox:
            ui_user_drag_box(block);
            break;
        case WimpEvt_KeyPress:
            ui_key_press(block);
            break;
        case WimpEvt_MenuSlct:
            ui_menu_selection(block);
            break;
        case WimpEvt_UserMsg:
        case WimpEvt_UserMsgRec:
            ui_user_message(block);
            break;
        case WimpEvt_UserMsgAck:
            ui_user_message_ack(block);
            break;
        default:
            break;
    }
...
void ui_poll(int dopoll)
{
    ...
    if (dopoll != 0) {
        ...
        do {
            ...
            if ((SnapshotPending != 0) || (SingleTasking != 0)) {
                break;
            }
        }
        ...
        if (--WithinUiPoll == 0) {
            if ((SoundSuspended != 0) && (EmuPaused == 0)) {
                sound_resume();
            }
        }
        ui_poll_epilogue();
...
ui_jam_action_t ui_jam_dialog(const char *format, ...)
{
    ...
    mbBox[0] = 0x100;
    mbBox[1] = (short)(w->vminx);
    mbBox[2] = (short)(w->vminy);
    mbBox[3] = (short)(w->vmaxx);
    mbBox[4] = (short)(w->vmaxy);
    ...
    mbBox[1] = 0;
    mbBox[2] = 0;
    mbBox[3] = ScreenMode.resx;
    mbBox[4] = ScreenMode.resy;
    MouseBoundingBox(((unsigned char*)mbBox) + 1);
...
{
    if (FullScreenMode == 0) {
        if ((ViceMachineCallbacks.display_speed == NULL) || (ViceMachineCallbacks.display_speed(percent, framerate, warp_flag) != 0)) {
            char buffer[32];
...
void ui_display_drive_track(unsigned int drive_number, unsigned int drive_base, unsigned int half_track_number)
{
    ...
    if (drive_number != DisplayDriveTrack) {
        return;
    }

    if ((icon = wimp_window_get_icon(EmuPane, Icon_Pane_TrkSec)) == NULL) {
        return;
    }
...
int ui_extend_image_dialog(void)
{
    ...
    err.errnum = 0;
    strcpy(err.errmess, SymbolStrings[Symbol_DlgExtend]);
    button = Wimp_ReportError(&err, 3, WimpTaskName);
