...
static void set_mem(int start_page, int end_page,
		    read_func_ptr_t read_func, store_func_ptr_t store_func,
		    BYTE *read_base, int base_mask)
{
	...
	for (i = start_page; i <= end_page; i++) {
	    _mem_read_tab[i] = read_func;
	    _mem_write_tab[i] = store_func;
...
void initialize_memory(void)
{
    ...
    set_mem(0, 0x03, read_ram, store_ram, ram, 0xffff);

    if (vic_memconf & VIC_BLK0)
	set_mem(0x04, 0x0f, read_ram, store_ram, ram, 0xffff);
    else
	set_mem(0x04, 0x0f, read_dummy, store_dummy, NULL, 0);

    set_mem(0x10, 0x1f, read_ram, store_ram, ram, 0xffff);

    if (vic_memconf & VIC_BLK1)
	set_mem(0x20, 0x3f, read_ram, store_ram, ram, 0xffff);
    else
	set_mem(0x20, 0x3f, read_dummy, store_dummy, NULL, 0);

    if (vic_memconf & VIC_BLK2)
	set_mem(0x40, 0x5f, read_ram, store_ram, ram, 0xffff);
    else
	set_mem(0x40, 0x5f, read_dummy, store_dummy, NULL, 0);

    if (vic_memconf & VIC_BLK3)
	set_mem(0x60, 0x7f, read_ram, store_ram, ram, 0xffff);
    else
	set_mem(0x60, 0x7f, read_dummy, store_dummy, NULL, 0);

    /* Setup character generator ROM at $8000-$8FFF. */
    set_mem(0x80, 0x8f, read_chargen, store_dummy, chargen_rom, 0x0fff);

    /* Setup VIC-I at $9000-$90FF. */
    set_mem(0x90, 0x90, read_vic, store_vic, NULL, 0);

    /* Setup VIAs at $9100-$93FF. */
    set_mem(0x91, 0x93, read_via, store_via, NULL, 0);
    ...
    /* Setup color memory at $9400-$9BFF.
       ...
       space. */
    set_mem(0x94, 0x9b, read_ram, store_ram, ram, 0xffff);
    set_mem(0x9c, 0x9f, read_dummy, store_dummy, NULL, 0);

    /* Setup extra RAM block at $A000-$BFFF. */
    if (vic_memconf & VIC_BLK5)
	set_mem(0xa0, 0xbf, read_ram, store_ram, ram, 0xffff);
    else
	set_mem(0xa0, 0xbf, read_dummy, store_dummy, NULL, 0);

    /* Setup BASIC ROM at $C000-$DFFF. */
    set_mem(0xc0, 0xdf, read_basic, store_dummy, basic_rom, 0x1fff);

    /* Setup Kernal ROM at $E000-$FFFF. */
    set_mem(0xe0, 0xff, read_kernal, store_dummy, kernal_rom, 0x1fff);

    /* Setup VIC memory configuration. */
