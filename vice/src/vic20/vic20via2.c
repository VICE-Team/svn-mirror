...
inline static void update_via2irq(void)
{
    ...
    if(irq && !(via2ifr & via2ier & 0x7f)) {
       fprintf(logfile, "via2: clk=%d, IRQ off\n", clk);
    }
    if(!irq && (via2ifr & via2ier & 0x7f)) {
       fprintf(logfile, "via2: clk=%d, IRQ on\n", clk);
    }
...
void REGPARM2 store_via2(ADDRESS addr, BYTE byte)
{
	...
	fprintf(logfile, "store via2[%x] %x, rmwf=%d, clk=%d, rclk=%d\n",
	...
	if (via2ier & (VIA_IM_CA1 | VIA_IM_CA2))
            update_via2irq();
	...
	if (via2ier & (VIA_IM_CB1 | VIA_IM_CB2))
            update_via2irq();
...
BYTE REGPARM1 read_via2(ADDRESS addr)
{
	...
	fprintf(logfile, "read_via2(%x) -> %02x, clk=%d\n", addr, retv, clk);
    return retv;
}
...
#ifdef VIA2_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "via2 timer A interrupt\n");
#endif

    if (!(via2[VIA_ACR] & 0x40)) {	/* one-shot mode */
#if 0				/* defined (VIA2_TIMER_DEBUG) */
	fprintf(logfile, "VIA2 Timer A interrupt -- one-shot mode: next int won't happen\n");
#endif
...
#ifdef VIA2_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "VIA2 timer B interrupt\n");
#endif
...
int via2_write_snapshot_module(snapshot_t * p)
{
    ...
    if (m == NULL)
        return -1;
/*
fprintf(logfile, "via2: write: clk=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
...
*/
    ...
    snapshot_module_write_byte(m, via2[VIA_T2LL]);
...
int via2_read_snapshot_module(snapshot_t * p)
{
    ...
    via2tal = word;
    via2[VIA_T1LL] = via2tal & 0xff;
    via2[VIA_T1LH] = (via2tal >> 8) & 0xff;
    snapshot_module_read_word(m, &word);
    via2tau = rclk + word + 2 /* 3 */ + TAUOFFSET;
    via2tai = rclk + word + 1;

    snapshot_module_read_byte(m, &via2[VIA_T2LL]);
    snapshot_module_read_word(m, &word);
...
/*
fprintf(logfile, "via2: read: clk=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
		clk, via2tai, via2tau, via2tbi, via2tbu);
fprintf(logfile, "     : ta=%d, tb=%d\n",via2ta() & 0xffff, via2tb() & 0xffff);
*/
