...
BYTE REGPARM1 read_via2(ADDRESS addr)
{
	...
	log_message(via2_log,
                    "read_via2(%x) -> %02x, clk=%d", addr, retv, clk);
    return retv;
}
...
int int_via2t1(long offset)
{
	...
	log_message(via2_log, "via2 timer A interrupt");
#endif

    if (!(via2[VIA_ACR] & 0x40)) {	/* one-shot mode */
#ifdef VIA2_TIMER_DEBUG
	log_message(via2_log, "VIA2 Timer A interrupt -- one-shot mode: next int won't happen");
#endif
...
#ifdef VIA2_TIMER_DEBUG
    if (app_resources.debugFlag)
	log_message(via2_log, "VIA2 timer B interrupt.");
#endif
