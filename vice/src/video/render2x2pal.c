	...
	for (y=yys;y<(yys+height);y++)
	{
		...
		if ((!(y & 1)) || doublescan)
		{
			...
			if (wfirst)
			{
				...
				*tmptrg++ = gamma_red[(red+red2)>>1] | gamma_grn[(grn+grn2)>>1] | gamma_blu[(blu+blu2)>>1];
			...
			if (wfirst)
			{
				...
				*tmptrg++ = gamma_red_fac[(red+red2)>>1] | gamma_grn_fac[(grn+grn2)>>1] | gamma_blu_fac[(blu+blu2)>>1];
			...
			for (x=0;x<(wfast << 3)+wstart+wend;x++)
			{
				...
				*tmptrg++ = gamma_red_fac[red] | gamma_grn_fac[grn] | gamma_blu_fac[blu];
				*tmptrg++ = gamma_red_fac[(red+red2)>>1] | gamma_grn_fac[(grn+grn2)>>1] | gamma_blu_fac[(blu+blu2)>>1];
