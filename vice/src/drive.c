...
static int  floppy_parse_name ( char *name, int length, char *realname, int *reallength, int *readmode, int *filetype, int *rl );
...
static int  floppy_parse_name(char *name, int length, char *ptr,
...
int     read_fs(void *flp, BYTE *data, int secondary)
{
	...
	if (info->dp) {
		...
		do {
			...
			if(fs_dirmask[i]=='*') {
			  if(!fs_dirmask[i+1]) { f=0; break; }	/* end mask */
			  while(*p && (*p != fs_dirmask[i+1])) p++;
			} else {
			  if(*p != fs_dirmask[i]) break;
			  p++;
			}
			if((!*p) && (!fs_dirmask[i+1])) { f=0; break; }
		    }
		} while(f);
...
int     open_fs(void *flp, char *name, int length, int secondary)
{
    ...
    */
	...
	if ((secondary != 0) || (fs_info[secondary].mode !=Read)) {
        ...
        /* test on wildcards */
        if(!(mask=strrchr(fsname,'/'))) mask=fsname;
