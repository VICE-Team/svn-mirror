/*
 * sidcart.c - MOS6581 (SID) cartridge emulation, hooks to actual implementation.
 *
 * Written by
...
BYTE REGPARM1 sidcart_read(WORD addr)
{
    return sid_read_chip(addr, 0);
}

BYTE REGPARM1 sidcart2_read(WORD addr)
{
    return sid_read_chip(addr, 1);
}

void REGPARM2 sidcart_store(WORD addr, BYTE byte)
{
    sid_store_chip(addr, byte, 0);
}

void REGPARM2 sidcart2_store(WORD addr, BYTE byte)
{
...
{
    native_sound_machine_init(psid, speed, cycles_per_sec);
...
{
    if (addr>0x1f)
        return native_sound_machine_read(psid, (WORD)(addr-0x20));
    else
...
{
    if (addr>0x1f)
        native_sound_machine_store(psid, (WORD)(addr-0x20), byte);
    else
				    ...
				    int interleave, int *delta_t)
{
  ...
  temp=sid_engine.calculate_samples(psid, pbuf, nr, interleave, delta_t);
  native_sound_machine_calculate_samples(psid, pbuf, nr, interleave, delta_t);
  return temp;
}
...
void sidcart_set_machine_parameter(long clock_rate)
{
#ifdef HAVE_CATWEASELMKIII
