...
BYTE read_mycia_(ADDRESS addr)
{
    ...
    switch (addr) {
      ...
      case CIA_PRA:		/* port A */
	READ_CIAPA
	byte = (byte & ~mycia[CIA_DDRA]) | (mycia[CIA_PRA] & mycia[CIA_DDRA]);
	return byte;
	break;

      case CIA_PRB:		/* port B */
	READ_CIAPB
	byte = (byte & ~mycia[CIA_DDRB]) | (mycia[CIA_PRB] & mycia[CIA_DDRB]);
        if ((mycia[CIA_CRA] | mycia[CIA_CRB]) & 0x02) {
	    update_mycia(rclk);
	    if (mycia[CIA_CRA] & 0x02) {
		byte &= 0xbf;
...
int mycia_write_snapshot_module(FILE * p)
{
    int byte;
...
int mycia_read_snapshot_module(FILE * p)
{
