...
struct mon_cmds mon_cmd_array[] = {
   ...
   { "@", "", CMD_DISK, STATE_ROL },
   { "displaytype", "dt", CMD_DISPLAYTYPE, STATE_INITIAL },
   ...
   { "readspace", "rs", CMD_READSPACE, STATE_INITIAL },
   ...
   { "verify", "", CMD_VERIFY, STATE_FNAME },
   { "watch", "w", CMD_WATCH, STATE_INITIAL },
   { "writespace", "ws", CMD_WRITESPACE, STATE_INITIAL },
...
{
   evaluate_default_addr(&addr, TRUE);
...
int cmd_get_token(int index)
{
   return mon_cmd_array[index].token;
}

int cmd_get_next_state(int index)
{
   return mon_cmd_array[index].next_state;
}

void print_help(int cmd_num)
{
...
void record_commands(char *filename)
{
   if (recording) {
...
void playback_commands(char *filename)
{
   FILE *fp;
   char string[255];
...
void execute_disk_command(char *cmd)
{
   int len, rc;
   serial_t *p;
   DRIVE *floppy;
