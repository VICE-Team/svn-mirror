...
static void mon_register_print(int mem)
{
    ...
    if (mon_register_get_val(mem, e_EMUL)) {
        mon_out("  ADDR AC BC XR YR EM SP DPRE PB DB NV-BDIZC");
    } else {
        mon_out("  ADDR AC BC XREG YREG EM STCK DPRE PB DB NVMXDIZC");
    }
    ...
    if (mon_register_get_val(mem, e_EMUL)) {
        mon_out(".;%04x %02x %02x %02x %02X %02x %02x %04x %02x %02x %d%d%c%d%d%d%d%d",
                  ...
                  mon_register_get_val(mem, e_EMUL),
        ...
        mon_out(".;%04x %02x %02x %04x %04X %02x %04x %04x %02x %02x %d%d%d%d%d%d%d%d",
                  addr_location(mon_register_get_val(mem, e_PC)),
                  mon_register_get_val(mem, e_A),
                  mon_register_get_val(mem, e_B),
...
static mon_reg_list_t *mon_register_list_get65816(int mem)
{
    ...
    mon_reg_list[2].val = (unsigned int)mon_register_get_val(mem, e_B);
    mon_reg_list[2].size = 8;
    mon_reg_list[2].flags = 0;
    mon_reg_list[2].next = &mon_reg_list[3];

    mon_reg_list[3].name = "XR";
    mon_reg_list[3].val = (unsigned int)mon_register_get_val(mem, e_X);
    mon_reg_list[3].size = 16;
    mon_reg_list[3].flags = 0;
    mon_reg_list[3].next = &mon_reg_list[4];

    mon_reg_list[4].name = "YR";
    mon_reg_list[4].val = (unsigned int)mon_register_get_val(mem, e_Y);
    ...
    mon_reg_list[6].name = "DPR";
    mon_reg_list[6].val = (unsigned int)mon_register_get_val(mem, e_DPR);
    mon_reg_list[6].size = 16;
    mon_reg_list[6].flags = 0;
    mon_reg_list[6].next = &mon_reg_list[7];

    mon_reg_list[7].name = "PBR";
    mon_reg_list[7].val = (unsigned int)mon_register_get_val(mem, e_PBR);
    ...
    mon_reg_list[9].name = "EMUL";
    mon_reg_list[9].val = (unsigned int)mon_register_get_val(mem, e_EMUL);
    mon_reg_list[9].size = 1;
    mon_reg_list[9].flags = 0;
    mon_reg_list[9].next = &mon_reg_list[10];

    mon_reg_list[10].name = "FL";
    ...
    mon_reg_list[12].name = "NVMXDIZC";
    mon_reg_list[12].val = (unsigned int)mon_register_get_val(mem, e_FLAGS);
    mon_reg_list[12].size = 8;
    mon_reg_list[12].flags = 1;
...
static void mon_register_list_set65816(mon_reg_list_t *reg_list, int mem)
{
    do {
        ...
        if (!strcmp(reg_list->name, "DBR")) {
            mon_register_set_val(mem, e_DBR, (WORD)(reg_list->val));
        }
        if (!strcmp(reg_list->name, "EMUL")) {
        ...
        if (!strcmp(reg_list->name, "NVMXDIZC")) {
            mon_register_set_val(mem, e_FLAGS, (WORD)(reg_list->val));
        }
