...
void REGPARM2 store_via(ADDRESS addr, BYTE byte)
{
	...
	if (viaier & (VIA_IM_CA1 | VIA_IM_CA2))
            update_viairq();
	...
	if (viaier & (VIA_IM_CB1 | VIA_IM_CB2))
            update_viairq();
...
BYTE REGPARM1 read_via(ADDRESS addr)
{
	...
	log_message(via_log,
                    "read_via(%x) -> %02x, clk=%d", addr, retv, clk);
    return retv;
}
...
int int_viat1(long offset)
{
	...
	log_message(via_log, "via timer A interrupt");
#endif

    if (!(via[VIA_ACR] & 0x40)) {	/* one-shot mode */
#ifdef VIA_TIMER_DEBUG
	log_message(via_log, "VIA Timer A interrupt -- one-shot mode: next int won't happen");
#endif
...
#ifdef VIA_TIMER_DEBUG
    if (app_resources.debugFlag)
	log_message(via_log, "VIA timer B interrupt.");
#endif
...
int via_write_snapshot_module(snapshot_t * p)
{
    ...
    snapshot_module_write_byte(m, via[VIA_T2LL]);
...
int via_read_snapshot_module(snapshot_t * p)
{
    ...
    viatal = word;
    via[VIA_T1LL] = viatal & 0xff;
    via[VIA_T1LH] = (viatal >> 8) & 0xff;
    snapshot_module_read_word(m, &word);
    viatau = rclk + word + 2 /* 3 */ + TAUOFFSET;
    viatai = rclk + word + 1;

    snapshot_module_read_byte(m, &via[VIA_T2LL]);
    snapshot_module_read_word(m, &word);
