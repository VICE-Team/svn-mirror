...
int resources_register_int(const resource_int_t *r)
{
    ...
    while (sp->name != NULL) {
        ...
        dp->type = RES_INTEGER;
        dp->factory_value = (resource_value_t)(long)(sp->factory_value);
                                       ...
                                       resource_value_t value)
{
    ...
    *event_data = lib_malloc(*data_size);
    ...
    if (r->type == RES_INTEGER)
        *(DWORD *)(*event_data + name_size) = (DWORD)(unsigned long)value;
    else
                                        ...
                                        resource_value_t value)
{
    ...
    switch (r->type) {
      case RES_INTEGER:
        status = (*r->set_func_int)((int)(long)value, r->param);
        break;
      case RES_STRING:
...
int resources_set_int(const char *name, int value)
{
    ...
    if (r->event_relevant == RES_EVENT_SAME && network_connected())
    {
        resource_record_event(r, (resource_value_t)(long)value);
        return 0;
    }
...
int resources_get_default_value(const char *name, void *value_return)
{
    ...
    switch (r->type) {
      case RES_INTEGER:
        *(int *)value_return = (int)(long)(r->factory_value);
        break;
      case RES_STRING:
...
int resources_set_defaults(void)
{
    ...
    for (i = 0; i < num_resources; i++) {
        switch (resources[i].type) {
          case RES_INTEGER:
            if ((*resources[i].set_func_int)((int)(long)(resources[i].factory_value),
                resources[i].param) < 0) {
        ...
        switch (resources[i].type) {
          case RES_INTEGER:
            if (resources[i].event_relevant == RES_EVENT_STRICT) {
                if ((*resources[i].set_func_int)((int)(long)(resources[i].event_strict_value),
                    resources[i].param) < 0)
                    return -1;
            }
            break;
...
static char *string_resource_item(int num, const char *delim)
{
    ...
    switch (resources[num].type) {
      case RES_INTEGER:
        ...
        line = lib_msprintf("%s=%d%s", resources[num].name, (int)(long)v, delim);
        break;
      case RES_STRING:
