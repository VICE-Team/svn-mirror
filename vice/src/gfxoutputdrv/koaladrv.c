...
static void koala_multicolorize_colormap(koala_data_t *source)
{
    ...
    for (i = 0; i < 200; i++) {
        for (j = 0; j < 160; j++) {
            source->colormap[(i * 320) + (j * 2) + 1] = source->colormap[(i * 320) + (j * 2)];
        }
    }
}
...
static void koala_check_and_correct_cell(koala_data_t *source, BYTE bgcolor)
{
    ...
    for (i = 0; i < 25; i++) {
        for (j = 0; j < 40; j++) {
            ...
            colors = koala_sort_colors_colormap(dest);
            bgcolor_included = 0;
            for (l = 0; l < 4; l++) {
            ...
            if (colors[4].amount !=0) {
                colors[4].color = 255;
                koala_color_to_nearest_colors_colormap(dest, colors);
                for (k = 0; k < 8; k++) {
                    for (l = 0; l < 8; l++) {
...
#define BGCOLOR_OFFSET 10002
    ...
    /* find out bgcolor */
    color_order = koala_sort_colors_colormap(source);
    ...
    for (i = 0; i < 25; i++) {
        for (j = 0; j < 40; j++) {
            for (k = 0; k < 8; k++) {
                ...
                for (l = 0; l < 4; l++) {
                    colorbyte = source->colormap[(i * 320 * 8) + (j * 8) + (k * 320) + (l * 2)];
                    if (k == 0 && l == 0) {
                        color1 = 255;
                        color2 = 255;
                        color3 = 255;
...
static int koala_vicii_text_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
    ...
    if (((regs[0x16] & 8) == 0) || ((regs[0x11] & 8) == 0)) {
        koala_smooth_scroll_borderize_colormap(data, (BYTE)(regs[0x20] & 0xf), (BYTE)((regs[0x16] & 8) ? 255 : regs[0x16] & 7), (BYTE)((regs[0x11]  & 8) ? 255 : regs[0x11] & 7));
    }
    return koala_render_and_save(data);
}

static int koala_vicii_extended_background_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
...
static int koala_vicii_multicolor_text_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE color0;
    BYTE color1;
...
static int koala_vicii_hires_bitmap_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
...
static int koala_vicii_multicolor_bitmap_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE color0;
    BYTE color1;
...
static int koala_vicii_save(screenshot_t *screenshot, const char *filename)
{
    ...
    switch (mc << 2 | eb << 1 | bm) {
        case 0:    /* normal text mode */
            return koala_vicii_text_mode_render(screenshot, filename);
            break;
        case 1:    /* hires bitmap mode */
            return koala_vicii_hires_bitmap_mode_render(screenshot, filename);
            break;
        case 2:    /* extended background mode */
            return koala_vicii_extended_background_mode_render(screenshot, filename);
            break;
        case 4:    /* multicolor text mode */
            return koala_vicii_multicolor_text_mode_render(screenshot, filename);
            break;
        case 5:    /* multicolor bitmap mode */
            return koala_vicii_multicolor_bitmap_mode_render(screenshot, filename);
            break;
...
static int koala_ted_text_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
    ...
    if (((regs[0x07] & 8) == 0) || ((regs[0x06] & 8) == 0)) {
        if (ted_lum_handling == NATIVE_SS_TED_LUM_DITHER) {
            koala_smooth_scroll_borderize_colormap(data, ted_lum_to_vicii_color(brdrcolor, brdrlum), (BYTE)((regs[0x07] & 8) ? 255  : regs[0x07] & 7), (BYTE)((regs[0x06] & 8) ? 255 : regs[0x06] & 7));
        } else {
            koala_smooth_scroll_borderize_colormap(data, ted_to_vicii_color(brdrcolor), (BYTE)((regs[0x07] & 8) ? 255 : regs[0x07]  & 7), (BYTE)((regs[0x06] & 8) ? 255 : regs[0x06] & 7));
        }
    }
    return koala_render_and_save(data);
}

static int koala_ted_extended_background_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
...
static int koala_ted_hires_bitmap_mode_render(screenshot_t *screenshot, const char *filename)
{
    BYTE bitmap;
    BYTE fgcolor;
    BYTE bgcolor;
...
static int koala_ted_save(screenshot_t *screenshot, const char *filename)
{
    ...
    switch (mc << 2 | eb << 1 | bm) {
        case 0:    /* normal text mode */
            return koala_ted_text_mode_render(screenshot, filename);
            break;
        case 1:    /* hires bitmap mode */
            return koala_ted_hires_bitmap_mode_render(screenshot, filename);
            break;
        case 2:    /* extended background mode */
            return koala_ted_extended_background_mode_render(screenshot, filename);
            break;
        case 4:    /* multicolor text mode */
...
static int koala_vic_save(screenshot_t *screenshot, const char *filename)
{
    ...
    if (ysize > 25) {
        if (oversize_handling == NATIVE_SS_OVERSIZE_SCALE) {
            data = koala_borderize_colormap(data, (BYTE)(regs[0xf] & 7));
            data = koala_scale_colormap(data);
        } else {
            data = koala_crop_and_borderize_colormap(data, (BYTE)(regs[0xf] & 7));
        }
    } else {
        data = koala_borderize_colormap(data, (BYTE)(regs[0xf] & 7));
    }
...
static int koala_crtc_save(screenshot_t *screenshot, const char *filename)
{
    ...
    if (data->xsize > 320 || data->ysize > 200) {
        if (oversize_handling == NATIVE_SS_OVERSIZE_SCALE) {
            data = koala_borderize_colormap(data, 0);
            data = koala_scale_colormap(data);
        } else {
            data = koala_crop_and_borderize_colormap(data, 0);
        }
    } else {
        data = koala_borderize_colormap(data, 0);
    }
