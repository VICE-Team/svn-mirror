...
#define AUDIO_FRAME_SAMPLES     (AUDIO_FREQ / VIDEO_FPS)
#define AUDIO_BUFFER_SIZE       ((AUDIO_FREQ * 2) / VIDEO_FPS)
...
static int audio_bitrate = 48000;
...
{
    printf("zmbvdrv_cmdline_options_init\n");
...
static int zmbvdrv_open_audio(int speed, int channels)
{
    ...
    zmbvdrv_audio_in.buffer = lib_malloc(AUDIO_BUFFER_SIZE * 2);
    if (zmbvdrv_audio_in.buffer == NULL) {
        log_error(LOG_DEFAULT, "zmbvdrv: Error allocating audio buffer (%u bytes)", (unsigned)AUDIO_BUFFER_SIZE * 2);
        return -1;
    }
    zmbvdrv_audio_in.size = AUDIO_BUFFER_SIZE;
#if 0
...
static int zmbvmovie_init_audio(int speed, int channels, soundmovie_buffer_t ** audio_in)
{
    LOG(("zmbvmovie_init_audio(speed:%d channels: %d)", speed, channels));
...
static int zmbvmovie_encode_audio(soundmovie_buffer_t *audio_in)
{
    int ret = 0;

    LOGFRAMES(("zmbvmovie_encode_audio(used:%d channels:%d)",  audio_in->used, audio_channels));
...
static void zmbvmovie_close(void)
{
    LOG(("zmbvmovie_close()"));
...
static soundmovie_funcs_t zmbvdrv_soundmovie_funcs = {
    zmbvmovie_init_audio,
    zmbvmovie_encode_audio,
    zmbvmovie_close
};
...
static void zmbvdrv_init_video(screenshot_t *screenshot)
{
    ...
    LOG(("zmbvdrv_init_video w:%d h:%d\n", video_width, video_height));
    video_init_done = 1;
...
static int zmbvdrv_init_file(void)
{
    ...
    if (zmbvdrv_open_audio(audio_bitrate, audio_channels) < 0) {
...
static int zmbvdrv_save(screenshot_t *screenshot, const char *filename)
{
    ...
    zavi = zmbv_avi_start(filename, video_width, video_height, VIDEO_FPS, AUDIO_FREQ);
    if (zavi == NULL) {
