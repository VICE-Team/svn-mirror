...
static alarm_t cia2_ta_alarm;
static alarm_t cia2_tb_alarm;
static alarm_t cia2_tod_alarm;
...
/* Fallback for "normal" use of the chip. In real operation the interrupt
   number can be replaced with the known constant I_CIA2FL (see 
   cia2_restore_int() below. */
#ifndef maincpu_set_nmi_clk
#define	maincpu_set_nmi_clk(value,num,clk)					\
		set_int(&maincpu_int_status,(num),(value),(clk))
#endif

/* Fallback for "normal" use of the chip. */
#ifndef cia2_restore_int
#define	cia2_restore_int(value)					\
...
/* The following is an attempt in rewriting the interrupt defines into 
   ...
   one could also remove IK_NMI as it is also know... */
...
static inline void my_set_int(int value, CLOCK rclk)
{
    ...
    if ((value)) {
        cia2int |= 0x80;
        maincpu_set_nmi_clk((I_CIA2FL), (IK_NMI), (rclk));
    } else {
        maincpu_set_nmi_clk((I_CIA2FL), 0, (rclk));
    }
}
...
void cia2_init(void)
{
    alarm_init(&cia2_ta_alarm, &maincpu_alarm_context, "CIA2TimerA",
               int_cia2ta);
    alarm_init(&cia2_tb_alarm, &maincpu_alarm_context, "CIA2TimerB",
               int_cia2tb);
    alarm_init(&cia2_tod_alarm, &maincpu_alarm_context, "CIA2TimeOfDay",
               int_cia2tod);
}
...
void reset_cia2(void)
{
    ...
    cia2todlatched = 0;
    cia2todstopped = 0;
    alarm_set(&cia2_tod_alarm, clk + cia2todticks);
...
int int_cia2ta(long offset)
{
	...
	if ( ( (cia2ier & CIA_IM_TA) &&
	    ...
	    if(offset > cia2_tal+1) {
            ...
            alarm_unset(&cia2_ta_alarm); /* do _not_ clear cia2_tai */
	}
    } else {
	my_unset_tai();
    }
...
log_message(cia2_log, "snap setting rdi to %d (rclk=%d)", cia2rdi, clk);
    ...
    alarm_set(&cia2_tod_alarm, clk + dword);
