...
static void tfe_set_tx_status(int ready,int error)
{
    ...
    if (new_status!=old_status) {
        ...
        log_message(tfe_log,"TX: set status Rdy4TxNOW=%d TxBidErr=%d", ready, error);
#endif
    }
}
...
static int tfe_activate_i(void)
{
    assert(tfe == NULL);
...
static int tfe_deactivate_i(void)
{
#ifdef TFE_DEBUG
...
#define return( _x_ )                                                                                                          \
    {                                                                                                                          \
        int retval = _x_;                                                                                                      \
                                                                                                                               \
        log_message(tfe_log, "%s correct_mac=%u, broadcast=%u, multicast=%u, hashed=%u, hash_index=%u",                        \
...
*/
int tfe_should_accept(unsigned char *buffer, int length, int *phashed, int *phash_index, int *pcorrect_mac, int *pbroadcast, int *pmulticast) 
{
...
static WORD tfe_receive(void)
{
    ...
    do {
        ...
        newframe = tfe_arch_receive(buffer, &len, &hashed, &hash_index, &rx_ok, &correct_mac, &broadcast, &crc_error);
...
#ifdef TFE_DEBUG_FRAMES
                log_message( tfe_log, "+++ tfe_receive(): *** hashed=%u, correct_mac=%u, broadcast=%u", hashed, correct_mac, broadcast);
#endif
            } else {
                /* determine ourself the type of frame */
                if (!tfe_should_accept(buffer, len, &hashed, &hash_index, &correct_mac, &broadcast, &multicast)) {
...
#ifdef TFE_DEBUG_RXTX_STATE
                log_message(tfe_log,"RX: recvd frame (length=%04x,status=%04x)", rx_length, ret_val);
#endif
            }
        }
    } while (!ready);
...
static void tfe_write_tx_buffer(BYTE value,int odd_address)
{
    ...
    if (tx_state != TFE_TX_READ_BUSST) {
#ifdef TFE_DEBUG_WARN_RXTX
        log_message(tfe_log, "WARNING! Ignoring TX Write without correct Transmit Condition! (odd=%d,value=%02x)", odd_address, value);
#endif
        ...
        log_message(tfe_log, "TX: %04x/%04x: %02x (buffer=%04x,odd=%d)", tx_count, tx_length, value, addr, odd_address);
#endif

        /* full frame transmitted? */
        if (tx_count == tx_length) {
#ifdef TFE_DEBUG_FRAMES
            log_message(tfe_log, "tfe_arch_transmit() called with:                 length=%4u and buffer %s", tx_length, debug_outbuffer(tx_length, &tfe_packetpage[TFE_PP_ADDR_TX_FRAMELOC]));
#endif
...
#ifdef TFE_DEBUG_WARN_RXTX
        log_message(tfe_log, "WARNING! RX Read without frame available! (odd=%d)", odd_address);
#endif
...
#ifdef TFE_DEBUG_RXTX_DATA
        log_message(tfe_log,"RX: %04x/%04x: %02x (buffer=%04x,odd=%d)", rx_count, rx_length + 4, value, addr, odd_address);
#endif
    ...
    switch (ppaddress) {
        ...
        case TFE_PP_ADDR_CC_RXCTL:
            if (tfe_recv_control!=content) {
                ...
                tfe_arch_recv_ctl(tfe_recv_broadcast, tfe_recv_mac, tfe_recv_multicast, tfe_recv_correct, tfe_recv_promiscuous, tfe_recv_hashfilter);
            }
            break;
        case TFE_PP_ADDR_CC_LINECTL:
            {
                ...
                if ((enable_tx != tx_enabled) || (enable_rx != rx_enabled)) {
                    ...
                    log_message(tfe_log, "line control: transmitter=%s receiver=%s", on_off_str(enable_tx), on_off_str(enable_rx));
                }
            }
            break;
...
#ifdef TFE_DEBUG_RXTX_STATE
                        log_message(tfe_log, "TX: Ready4TXNow set! (%04x)", bus_status);
#endif
                    } else {
#ifdef TFE_DEBUG_RXTX_STATE
                        log_message(tfe_log, "TX: waiting for Ready4TXNow! (%04x)", bus_status);
#endif
                    }
                }
            }
...
static WORD tfe_read_register(WORD ppaddress)
{
    ...
    if (ppaddress < 0x100) {
        ...
        log_message(tfe_log, "Read Control Register %04x: %04x (reg=%02x)", ppaddress, value, regNum);
#endif
 
        /* reserved register? */
        if ((regNum == 0x01) || (regNum == 0x11) || (regNum > 0x19)) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Read reserved Control Register %04x (reg=%02x)", ppaddress, regNum);
#endif
...
#endif
        log_message(tfe_log, "Read  Status  Register %04x: %04x (reg=%02x)", ppaddress, value, regNum);
#endif

        /* reserved register? */
        if ((regNum == 0x02) || (regNum == 0x06) || (regNum == 0x0a) || (regNum == 0x0e) || (regNum == 0x1a) || (regNum == 0x1e)) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Read reserved Status Register %04x (reg=%02x)", ppaddress, regNum);
#endif
            /* real HW returns 0x0300 in reserved register range */
            return 0x0300;
        }
...
#ifdef TFE_DEBUG_REGISTERS
            log_message(tfe_log, "Read  TX Cmd  Register %04x: %04x", ppaddress, value);
#endif
        } else if (ppaddress == 0x146) {
#ifdef TFE_DEBUG_REGISTERS
            log_message(tfe_log, "Read  TX Len  Register %04x: %04x", ppaddress, value);
#endif
        } else {
            /* reserved range */
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Read reserved Initiate Transmit Register %04x", ppaddress);
#endif
            /* real HW returns 0x0300 in reserved register range */
            return 0x0300;
        }
        ...
        if (ppaddress >= 0x15e) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Read reserved Address Filter Register %04x", ppaddress);
#endif
            /* real HW returns 0x0300 in reserved register range */
            return 0x0300;
        }
        ...
        */
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Read reserved Register %04x", ppaddress);
#endif
        return 0x0300;
    } else if (ppaddress < 0xa00) {
        /* --- range from 0x400 .. 0x9ff --- RX Frame */
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Read from RX Buffer Range %04x", ppaddress);
#endif
        return 0x0000;
    } else {
...
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Read from TX Buffer Range %04x", ppaddress);
#endif
        return 0x0000;
    }
...
void tfe_write_register(WORD ppaddress,WORD value)
{
    ...
    if (ppaddress < 0x100) {
        ...
        if (ignore) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Ignoring write to read only/reserved Bus Interface Register %04x", ppaddress);
#endif
            return;
        }
    } else if (ppaddress < 0x120) {
...
#ifdef TFE_DEBUG_REGISTERS
        log_message(tfe_log, "Write Control Register %04x: %04x (reg=%02x)", ppaddress, value, regNum);
#endif

        /* invalid register? -> ignore! */
        if ((regNum == 0x01) || (regNum == 0x11) || (regNum > 0x19)) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Ignoring write to reserved Control Register %04x (reg=%02x)", ppaddress, regNum);
#endif
            return;
        }
    } else if (ppaddress < 0x140) {
...
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Ignoring write to read-only Status Register %04x", ppaddress);
#endif
        return;
    } else if (ppaddress < 0x150) {
        ...
        if (ppaddress == 0x144) {
            ...
            value &= 0x33ff;
#ifdef TFE_DEBUG_REGISTERS
            log_message(tfe_log, "Write TX Cmd  Register %04x: %04x", ppaddress, value);
#endif
        } else if (ppaddress == 0x146) {
            ...
            value &= 0x0fff;
#ifdef TFE_DEBUG_REGISTERS
            log_message(tfe_log, "Write TX Len  Register %04x: %04x", ppaddress, value);
#endif
        } else if ((ppaddress < 0x144) || (ppaddress > 0x147)) {
            /* reserved range */
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Ignoring write to reserved Initiate Transmit Register %04x", ppaddress);
#endif
            return;
        }
    } else if (ppaddress < 0x160) {
        ...
        if (ppaddress >= 0x15e) {
#ifdef TFE_DEBUG_WARN_REG
            log_message(tfe_log, "WARNING! Ingoring write to reserved Address Filter Register %04x", ppaddress);
#endif
            return;
        }
    } else if (ppaddress < 0x400) {
...
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Ingoring write to reserved Register %04x", ppaddress);
#endif
        return;
    } else if (ppaddress < 0xa00) {
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Ignoring write to RX Buffer Range %04x", ppaddress);
#endif
        return;
    } else {
#ifdef TFE_DEBUG_WARN_REG
        log_message(tfe_log, "WARNING! Ignoring write to TX Buffer Range %04x", ppaddress);
#endif
        return;
    }
...
#ifdef TFE_DEBUG_STORE
        log_message(tfe_log, "before writing to PP Ptr: $%04X <= $%04X.", ppaddress, word_value);
#endif
...
#ifdef TFE_DEBUG_STORE
        log_message(tfe_log, "after  writing to PP Ptr: $%04X <= $%04X.", ppaddress, word_value);
#endif
    }
