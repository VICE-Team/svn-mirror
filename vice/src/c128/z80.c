...
#define ADDHLREG(reg_valh, reg_vall)                             \
  do {                                                           \
      DWORD tmp;                                                 \
                                                                 \
      tmp = (reg_h << 8) + reg_l + ((reg_valh << 8) + reg_vall); \
      reg_h = tmp >> 8;                                          \
      reg_l = tmp & 0xff;                                        \
...
#define LDHLIND(reg_val)                        \
  do {                                          \
      CLK += 13;                                \
      reg_l = LOAD(reg_val);                    \
      reg_h = LOAD(reg_val + 1);                \
      CLK += 3;                                 \
      INC_PC(3);                                \
  } while (0)
...
#define SBCHLSP()                                                         \
  do {                                                                    \
      DWORD tmp;                                                          \
                                                                          \
      tmp = (reg_h << 8) + reg_l - reg_sp - (reg_f & C_FLAG);             \
      reg_f = N_FLAG;                                                     \
      LOCAL_SET_CARRY(tmp & 0x10000);                                     \
...
inline void opcode_cb(BYTE ip1, BYTE ip2, BYTE ip3, WORD ip12)
{
    switch (ip1) {
      case 0x80: /* RES B 0 */
        RES(reg_b, 0);
