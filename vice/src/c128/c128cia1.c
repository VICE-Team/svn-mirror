...
/*
 * ../../src/c128/c128cia1.c
 * This file is generated from ../../src/cia-tmpl.c and ../../src/c128/c128cia1.def,
 * Do not edit!
 */
/*
...
int cia1_dump(FILE * p)
{
    int byte;

    snapshot_write_module_header(p, "CIA1", 
			CIA_DUMP_VER_MAJOR, CIA_DUMP_VER_MINOR);
	
    update_cia1(clk);

    snapshot_write_byte(p, cia1[CIA_PRA]);
    snapshot_write_byte(p, cia1[CIA_PRB]);
    snapshot_write_byte(p, cia1[CIA_DDRA]);
    snapshot_write_byte(p, cia1[CIA_DDRB]);
    snapshot_write_word(p, cia1_tac);
    snapshot_write_word(p, cia1_tbc);
    snapshot_write_byte(p, cia1[CIA_TOD_TEN]);
    snapshot_write_byte(p, cia1[CIA_TOD_SEC]);
    snapshot_write_byte(p, cia1[CIA_TOD_MIN]);
    snapshot_write_byte(p, cia1[CIA_TOD_HR]);
    snapshot_write_byte(p, cia1[CIA_SDR]);
    snapshot_write_byte(p, cia1[CIA_ICR]);
    snapshot_write_byte(p, cia1[CIA_CRA]);
    snapshot_write_byte(p, cia1[CIA_CRB]);

    snapshot_write_word(p, cia1_tal);
    snapshot_write_word(p, cia1_tbl);
    snapshot_write_byte(p, peek_cia1(CIA_ICR));
    snapshot_write_byte(p, (cia1_tat ? 0x40 : 0) | (cia1_tbt ? 0x80 : 0));
    snapshot_write_byte(p, cia1sr_bits);
    snapshot_write_byte(p, cia1todalarm[0]);
    snapshot_write_byte(p, cia1todalarm[1]);
    snapshot_write_byte(p, cia1todalarm[2]);
    snapshot_write_byte(p, cia1todalarm[3]);
    ...
    snapshot_write_byte(p, 
		(cia1todlatched ? 1 : 0) | (cia1todstopped ? 2 : 0));
    snapshot_write_byte(p, cia1todlatch[0]);
    snapshot_write_byte(p, cia1todlatch[1]);
    snapshot_write_byte(p, cia1todlatch[2]);
    snapshot_write_byte(p, cia1todlatch[3]);

    snapshot_write_dword(p, maincpu_int_status.alarm_clk[A_CIA1TOD] - clk);

    return 0;
}

int cia1_undump(FILE * p)
{
    ...
    if(strcmp(name, "CIA1") || vmajor != CIA_DUMP_VER_MAJOR) return -1;
	
    snapshot_read_byte(p, &cia1[CIA_PRA]);
    snapshot_read_byte(p, &cia1[CIA_PRB]);
    snapshot_read_byte(p, &cia1[CIA_DDRA]);
    snapshot_read_byte(p, &cia1[CIA_DDRB]);
    ...
    snapshot_read_word(p, &word);
    cia1_tbc = word;
    snapshot_read_byte(p, &cia1[CIA_TOD_TEN]);
    snapshot_read_byte(p, &cia1[CIA_TOD_SEC]);
    snapshot_read_byte(p, &cia1[CIA_TOD_MIN]);
    snapshot_read_byte(p, &cia1[CIA_TOD_HR]);
    snapshot_read_byte(p, &cia1[CIA_SDR]);
    {
    ...
    snapshot_read_byte(p, &cia1[CIA_ICR]);
    snapshot_read_byte(p, &cia1[CIA_CRA]);
    snapshot_read_byte(p, &cia1[CIA_CRB]);
    ...
    snapshot_read_byte(p, &cia1todalarm[0]);
    snapshot_read_byte(p, &cia1todalarm[1]);
    snapshot_read_byte(p, &cia1todalarm[2]);
    snapshot_read_byte(p, &cia1todalarm[3]);
    ...
    snapshot_read_byte(p, &cia1todlatch[0]);
    snapshot_read_byte(p, &cia1todlatch[1]);
    snapshot_read_byte(p, &cia1todlatch[2]);
    snapshot_read_byte(p, &cia1todlatch[3]);
    ...
    if((cia1[CIA_CRB] & 0x41) == 0x41) cia1_tbs = CIAT_COUNTTA;
