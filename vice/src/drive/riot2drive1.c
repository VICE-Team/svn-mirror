...
#define my_set_irq(fl, clk)	\
	do { \
	printf("set_int_d1(%d)\n",(fl)); \
        set_int(&drive1_context.cpu.int_status,I_RIOTD1FL,(fl) ? IK_IRQ : 0, (clk)) \
	; } while(0)
*/
#define my_set_irq(fl, clk)	\
        set_int(&drive1_context.cpu.int_status,I_RIOTD1FL,(fl) ? IK_IRQ : 0, (clk))
...
#define my_restore_irq(fl)	\
        set_int_noclk(&drive1_context.cpu.int_status,I_RIOTD1FL,(fl) ? IK_IRQ : 0)
...
{
    parallel_drv1_set_nrfd((char)
	(((pa & 0x4) == 0)
	|| ((pa & 1) && !atn_active)
	|| (((pa & 1) == 0) && atn_active)
	));
    parallel_drv1_set_ndac((char)
	((pa & 0x2)
	|| (((pa & 0x1) == 0) && atn_active)
	));
}
