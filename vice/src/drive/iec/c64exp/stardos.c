...
int stardos_load(const char *name)
{
    DBG(("stardos_load <%s>\n", name));
...
static BYTE stardos_read(drive_context_t *drv, WORD addr)
{
    DBG(("stardos_read <%04x> <%02x>\n", addr, stardos_rom[addr & 0x1fff]));
    return stardos_rom[addr & 0x1fff];
}

void stardos_mem_init(struct drive_context_s *drv, unsigned int type)
{
    drivecpud_context_t *cpud = drv->cpud;

    DBG(("stardos_mem_init <type:%d> <sc:%d>\n", type, drv->drive->stardos));
 ...
 /* case DRIVE_TYPE_1570:
    ...
    case DRIVE_TYPE_1571CR: */
        drivemem_set_func(cpud, 0xA0, 0xC0, stardos_read, NULL, NULL, stardos_rom, 0xa000bffd);
        break;
    default:
        break;
    }
...
void stardos_init(drive_context_t *drv)
{
}

void stardos_reset(drive_context_t *drv)
{
}
