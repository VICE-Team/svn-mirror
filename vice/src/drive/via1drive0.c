...
#if 0	/* DEBUG */
    static int irq = 0;
    if(irq && !(via1d0ifr & via1d0ier & 0x7f)) {
       fprintf(logfile, "via1d0: clk=%d, IRQ off\n", clk);
    }
    if(!irq && (via1d0ifr & via1d0ier & 0x7f)) {
       fprintf(logfile, "via1d0: clk=%d, IRQ on\n", clk);
    }
    irq = (via1d0ifr & via1d0ier & 0x7f);
#endif
...
void REGPARM2 store_via1d0(ADDRESS addr, BYTE byte)
{
	...
	fprintf(logfile, "store via1d0[%x] %x, rmwf=%d, clk=%d, rclk=%d\n",
...
BYTE REGPARM1 read_via1d0(ADDRESS addr)
{
	...
	fprintf(logfile, "read_via1d0(%x) -> %02x, clk=%d\n", addr, retv, drive_clk[0]);
    return retv;
}
...
#ifdef VIA1D0_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "via1d0 timer A interrupt\n");
#endif

    if (!(via1d0[VIA_ACR] & 0x40)) {	/* one-shot mode */
#if 0				/* defined (VIA1D0_TIMER_DEBUG) */
	fprintf(logfile, "VIA1D0 Timer A interrupt -- one-shot mode: next int won't happen\n");
#endif
...
#ifdef VIA1D0_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "VIA1D0 timer B interrupt\n");
#endif
...
int via1d0_write_snapshot_module(snapshot_t * p)
{
    ...
    if (m == NULL)
        return -1;
/*
fprintf(logfile, "via1d0: write: drive_clk[0]=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
		drive_clk[0], via1d0tai, via1d0tau, via1d0tbi, via1d0tbu);
...
/*
fprintf(logfile, "via1d0: read: drive_clk[0]=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
		drive_clk[0], via1d0tai, via1d0tau, via1d0tbi, via1d0tbu);
fprintf(logfile, "     : ta=%d, tb=%d\n",via1d0ta() & 0xffff, via1d0tb() & 0xffff);
*/
