/*
 * via1drive0.c - VIA1 emulation in the 1541, 1541II, 1571 and 2031 disk drive.
 *
 * Written by
...
#define myvia via1d0
#define myvia_init via1d0_init

#define I_MYVIAFL I_VIA1D0FL
#define MYVIA_INT VIA1D0_INT
#define MYVIA_NAME "Drive0Via1"
...
static void undump_pra(BYTE byte)
{
    ...
    if (drive[0].type == DRIVE_TYPE_2031) {
        parallel_drv0_set_bus(parieee_is_out ? byte : 0xff);
    }
...
static void undump_prb(BYTE byte)
{
    if (iec_info != NULL) {
        ...
        if (drive[0].type == DRIVE_TYPE_2031) {
            ...
            parallel_drv0_set_bus(parieee_is_out ? oldpa : 0xff);
