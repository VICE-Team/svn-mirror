...
void drive1_mem_init(int type)
{
    ...
    if (type == DRIVE_TYPE_1571) {
        ...
        read_func_nowatch[0x8] = read_wd1770d1;
        store_func_nowatch[0x8] = store_wd1770d1;
    }
...
inline void drive1_cpu_wake_up(void)
{
	...
	log_message(drive[1].log, "Skipping cycles.");
	last_clk = clk;
    }
}
...
CLOCK drive1_cpu_prevent_clk_overflow(CLOCK sub)
{
	...
	wd1770d1_prevent_clk_overflow(sub);
    }
...
/* Handle a ROM trap. */
inline static int drive1_trap_handler(void)
{
    ...
    if (MOS6510_REGS_GET_PC(&drive1_cpu_regs) == 0xc0be) {
        /* 1581 job code */
        MOS6510_REGS_SET_PC(&drive1_cpu_regs, 0xc197);
        wd1770_handle_job_code(1);
        return 0;
    }
    return 1;
}
...
#define ROM_TRAP_HANDLER() drive1_trap_handler()

#define CALLER e_disk9_space
...
void drive1_set_bank_base(void)
{
    JUMP(reg_pc);
}
...
static char snap_module_name[] = "DRIVECPU1";
#define SNAP_MAJOR 0
#define SNAP_MINOR 0

int drive1_cpu_write_snapshot_module(snapshot_t *s)
{
    snapshot_module_t *m;
    ...
    if (0
        || snapshot_module_write_dword(m, (DWORD) drive_clk[1]) < 0
    ...
    if (drive[1].type == DRIVE_TYPE_1581) {
        if (snapshot_module_write_byte_array(m, drive1_ram, 0x2000) < 0)
            goto fail;
    }
...
int drive1_cpu_read_snapshot_module(snapshot_t *s)
{
    BYTE major, minor;
    snapshot_module_t *m;
    ...
    if (0
        || snapshot_module_read_dword(m, &drive_clk[1]) < 0
    ...
    if (interrupt_read_snapshot(&drive1_int_status, m) < 0)
        goto fail;
    ...
    if (drive[1].type == DRIVE_TYPE_1541
        ...
        || drive[1].type == DRIVE_TYPE_1571
        || drive[1].type == DRIVE_TYPE_2031) {
        if (snapshot_module_read_byte_array(m, drive1_ram, 0x800) < 0)
            goto fail;
    }
