...
static log_t via1d1_log = LOG_ERR;
...
void reset_via1d1(void)
{
    ...
    if (via1d1_log == LOG_ERR)
        via1d1_log = log_open("VIA1D1");
...
BYTE REGPARM1 read_via1d1(ADDRESS addr)
{
	...
	log_message(via1d1_log,
                    "read_via1d1(%x) -> %02x, clk=%d", addr, retv, drive_clk[1]);
    return retv;
}
...
int int_via1d1t1(long offset)
{
	...
	log_message(via1d1_log, "via1d1 timer A interrupt");
#endif

    if (!(via1d1[VIA_ACR] & 0x40)) {	/* one-shot mode */
#ifdef VIA1D1_TIMER_DEBUG
	log_message(via1d1_log, "VIA1D1 Timer A interrupt -- one-shot mode: next int won't happen");
#endif
...
#ifdef VIA1D1_TIMER_DEBUG
    if (app_resources.debugFlag)
	log_message(via1d1_log, "VIA1D1 timer B interrupt.");
#endif
