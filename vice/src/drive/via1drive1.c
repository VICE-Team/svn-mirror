...
inline static void update_via1d1irq(void)
{
    ...
    if(irq && !(via1d1ifr & via1d1ier & 0x7f)) {
       fprintf(logfile, "via1d1: clk=%d, IRQ off\n", clk);
    }
    if(!irq && (via1d1ifr & via1d1ier & 0x7f)) {
       fprintf(logfile, "via1d1: clk=%d, IRQ on\n", clk);
    }
...
void REGPARM2 store_via1d1(ADDRESS addr, BYTE byte)
{
	...
	fprintf(logfile, "store via1d1[%x] %x, rmwf=%d, clk=%d, rclk=%d\n",
	...
	if (via1d1ier & (VIA_IM_CA1 | VIA_IM_CA2))
            update_via1d1irq();
	...
	if (via1d1ier & (VIA_IM_CB1 | VIA_IM_CB2))
            update_via1d1irq();
...
BYTE REGPARM1 read_via1d1(ADDRESS addr)
{
	...
	fprintf(logfile, "read_via1d1(%x) -> %02x, clk=%d\n", addr, retv, drive_clk[1]);
    return retv;
}
...
#ifdef VIA1D1_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "via1d1 timer A interrupt\n");
#endif

    if (!(via1d1[VIA_ACR] & 0x40)) {	/* one-shot mode */
#if 0				/* defined (VIA1D1_TIMER_DEBUG) */
	fprintf(logfile, "VIA1D1 Timer A interrupt -- one-shot mode: next int won't happen\n");
#endif
...
#ifdef VIA1D1_TIMER_DEBUG
    if (app_resources.debugFlag)
	fprintf(logfile, "VIA1D1 timer B interrupt\n");
#endif
...
int via1d1_write_snapshot_module(snapshot_t * p)
{
    ...
    if (m == NULL)
        return -1;
/*
fprintf(logfile, "via1d1: write: drive_clk[1]=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
...
*/
    ...
    snapshot_module_write_byte(m, via1d1[VIA_T2LL]);
...
int via1d1_read_snapshot_module(snapshot_t * p)
{
    ...
    via1d1tal = word;
    via1d1[VIA_T1LL] = via1d1tal & 0xff;
    via1d1[VIA_T1LH] = (via1d1tal >> 8) & 0xff;
    snapshot_module_read_word(m, &word);
    via1d1tau = rclk + word + 2 /* 3 */ + TAUOFFSET;
    via1d1tai = rclk + word + 1;

    snapshot_module_read_byte(m, &via1d1[VIA_T2LL]);
    snapshot_module_read_word(m, &word);
...
/*
fprintf(logfile, "via1d1: read: drive_clk[1]=%d, tai=%d, tau=%d\n"
       "     : tbi=%d, tbu=%d\n",
		drive_clk[1], via1d1tai, via1d1tau, via1d1tbi, via1d1tbu);
fprintf(logfile, "     : ta=%d, tb=%d\n",via1d1ta() & 0xffff, via1d1tb() & 0xffff);
*/
