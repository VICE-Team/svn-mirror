...
int via1d1_dump(FILE * p)
{
    ...
    snapshot_write_module_header(p, "VIA1D1",
                        VIA_DUMP_VER_MAJOR, VIA_DUMP_VER_MINOR);

    snapshot_write_byte(p, via1d1[VIA_PRA]);
    snapshot_write_byte(p, via1d1[VIA_DDRA]);
    snapshot_write_byte(p, via1d1[VIA_PRB]);
    snapshot_write_byte(p, via1d1[VIA_DDRB]);

    snapshot_write_word(p, via1d1tal);
    snapshot_write_word(p, via1d1ta());
    snapshot_write_byte(p, via1d1tbl);
    snapshot_write_word(p, via1d1tb());

    snapshot_write_byte(p, via1d1[VIA_SR]);
    snapshot_write_byte(p, via1d1[VIA_ACR]);
    snapshot_write_byte(p, via1d1[VIA_PCR]);

    snapshot_write_byte(p, via1d1ifr);
    snapshot_write_byte(p, via1d1ier);

						/* FIXME! */
    snapshot_write_byte(p, (((via1d1pb7 ^ via1d1pb7x) | via1d1pb7o) ? 0x80 : 0));	
    snapshot_write_byte(p, 0);			/* SRHBITS */

    return 0;
}

int via1d1_undump(FILE * p)
{
    ...
    if(strcmp(name, "VIA1D1") || vmajor != VIA_DUMP_VER_MAJOR) return -1;

    snapshot_read_byte(p, &via1d1[VIA_PRA]);
    snapshot_read_byte(p, &via1d1[VIA_DDRA]);
    snapshot_read_byte(p, &via1d1[VIA_PRB]);
    snapshot_read_byte(p, &via1d1[VIA_DDRB]);
    {
        addr = VIA_DDRA;
	byte = via1d1[addr];
    ...
    via1d1tal = word;
    snapshot_read_word(p, &word);
    via1d1tau = rclk + word + 1 /* 3 */ + TAUOFFSET;
    via1d1tai = rclk + word /* + 2 */;
    drive1_set_alarm_clk(A_VIA1D1T1, via1d1tai);

    snapshot_read_byte(p, &byte);
    via1d1tbl = byte;
    snapshot_read_word(p, &word);
    via1d1tbu = rclk + word + 1 /* 3 */;
    via1d1tbi = rclk + word /* + 2 */;
    drive1_set_alarm(A_VIA1D1T2, via1d1tbi);

    snapshot_read_byte(p, &via1d1[VIA_SR]);
    { 
	addr = via1d1[VIA_SR];
	byte = via1d1[addr];
	
    }
    snapshot_read_byte(p, &via1d1[VIA_ACR]);
    snapshot_read_byte(p, &via1d1[VIA_PCR]);
    {
	addr = via1d1[VIA_PCR];
	byte = via1d1[addr];
