...
static int int_cia1581d1ta(long offset);
static int int_cia1581d1tb(long offset);
static int int_cia1581d1tod(long offset);
...
static alarm_t cia1581d1_ta_alarm;
static alarm_t cia1581d1_tb_alarm;
static alarm_t cia1581d1_tod_alarm;
...
static log_t cia1581d1_log = LOG_ERR;
...
static inline void my_set_int(int value, CLOCK rclk)
{
    ...
    if(cia1581d1_debugFlag) {
        log_message(cia1581d1_log, "set_int(rclk=%d, int=%d, d=%d pc=).",
           rclk,(int_num),(value));
    }
#endif
    if ((value)) {
...
{
    cia1581d1_tai = tai_clk;
    alarm_set(&cia1581d1_ta_alarm, tai_clk);
}

static inline void my_unset_tai(void) 
{
    cia1581d1_tai = -1;							\
    alarm_unset(&cia1581d1_ta_alarm);
}

static inline void my_set_tbi_clk(CLOCK tbi_clk) 
{
    cia1581d1_tbi = tbi_clk;
    alarm_set(&cia1581d1_tb_alarm, tbi_clk);
}

static inline void my_unset_tbi(void)
{
    cia1581d1_tbi = -1;
    alarm_unset(&cia1581d1_tb_alarm);
}
...
#ifdef CIA1581D1_TIMER_DEBUG
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "update: rclk=%d, tas=%d, tau=%d, tal=%u, ",
...
#ifdef CIA1581D1_TIMER_DEBUG
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "aic=%d, tac-> %u, tau-> %d tmp=%u",
                    added_int_clk, cia1581d1_tac, cia1581d1_tau, tmp);
#endif
...
#ifdef CIA1581D1_TIMER_DEBUG
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "tbc-> %u, tbu-> %d, int %02x ->",
...
#ifdef CIA1581D1_TIMER_DEBUG
		if (cia1581d1_debugFlag)
		    log_message(cia1581d1_log, "TA Reading ICR at rclk=%d prevented IRQ.",
			   rclk);
#endif
	    } else {
		if (cia1581d1ier & cia1581d1int & 0x7f) {
...
#ifdef CIA1581D1_TIMER_DEBUG
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "%02x.", cia1581d1int);
#endif
...
void cia1581d1_init(void)
{
    alarm_init(&cia1581d1_ta_alarm, &drive1_alarm_context, "CIA1581D1TimerA",
               int_cia1581d1ta);
    alarm_init(&cia1581d1_tb_alarm, &drive1_alarm_context, "CIA1581D1TimerB",
               int_cia1581d1tb);
    alarm_init(&cia1581d1_tod_alarm, &drive1_alarm_context, "CIA1581D1TimeOfDay",
               int_cia1581d1tod);
}
...
void reset_cia1581d1(void)
{
    ...
    if (cia1581d1_log == LOG_ERR)
        cia1581d1_log = log_open("CIA1581D1");
    ...
    alarm_set(&cia1581d1_tod_alarm, drive_clk[1] + cia1581d1todticks);
...
void REGPARM2 store_cia1581d1(ADDRESS addr, BYTE byte)
{
...
#ifdef CIA1581D1_TIMER_DEBUG
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "store cia1581d1[%02x] %02x @ clk=%d",
    ...
    iec_fast_drive_write(cia1581d1[CIA_SDR]);
		...
		if(cia1581d1sr_bits < 16) {
	        ...
	        if (cia1581d1_debugFlag)
	    	    log_message(cia1581d1_log, "start SDR rclk=%d.", rclk);
#endif
  	    }
	}
	break;
...
#if defined (CIA1581D1_TIMER_DEBUG)
	if (cia1581d1_debugFlag)
	    log_message(cia1581d1_log, "CIA1581D1 set CIA_ICR: 0x%x.", byte);
#endif
...
#if defined(CIA1581D1_TIMER_DEBUG)
	if (cia1581d1_debugFlag)
	    log_message(cia1581d1_log, "    set icr: ifr & ier & 0x7f -> %02x, int=%02x.",
...
#if defined (CIA1581D1_TIMER_DEBUG)
	if (cia1581d1_debugFlag)
	    log_message(cia1581d1_log, "CIA1581D1 set CIA_CRA: 0x%x (clk=%d, pc=, tal=%u, tac=%u).",
...
#if defined (CIA1581D1_TIMER_DEBUG)
	if (cia1581d1_debugFlag)
	    log_message(cia1581d1_log, "    -> tas=%d, tau=%d.", cia1581d1_tas, cia1581d1_tau);
#endif
...
#if defined (CIA1581D1_TIMER_DEBUG)
	if (cia1581d1_debugFlag)
	    log_message(cia1581d1_log, "CIA1581D1 set CIA_CRB: 0x%x (clk=%d, pc=, tbl=%u, tbc=%u).",
...
#if defined(CIA1581D1_TIMER_DEBUG)
		if (cia1581d1_debugFlag)
		    log_message(cia1581d1_log, "rclk=%d force load: set tbu alarm to %d.", rclk, cia1581d1_tbu);
#endif
		my_set_tbi_clk(cia1581d1_tbu + 1);
	    }
	}
...
#if defined(CIA1581D1_TIMER_DEBUG)
		if (cia1581d1_debugFlag)
		    log_message(cia1581d1_log, "rclk=%d start timer: set tbu alarm to %d.", rclk, cia1581d1_tbu);
#endif
		my_set_tbi_clk(cia1581d1_tbu + 1);
...
#if defined(CIA1581D1_TIMER_DEBUG)
		if (cia1581d1_debugFlag)
		    log_message(cia1581d1_log, "rclk=%d stop timer: set tbu alarm.", rclk);
#endif
		my_unset_tbi();
		cia1581d1_tbu = 0;
		if (!(byte & 0x10)) {
...
BYTE REGPARM1 read_cia1581d1(ADDRESS addr)
{
    ...
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log, "read cia1581d1[%x] returns %02x @ clk=%d.",
...
#ifdef CIA1581D1_TIMER_DEBUG
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log, "CIA1581D1 read intfl: rclk=%d, alarm_ta=%d, alarm_tb=%d",
...
#ifdef CIA1581D1_TIMER_DEBUG
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log,
                            "read intfl gives cia1581d1int=%02x -> %02x "
...
BYTE REGPARM1 peek_cia1581d1(ADDRESS addr)
{
      ...
      case CIA_ICR:		/* Interrupt Flag Register */
	{
	    ...
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log,
                            "CIA1581D1 read intfl: rclk=%d, alarm_ta=%d, alarm_tb=%d.",
                            rclk, drive1_int_status.alarm_clk[A_CIA1581D1TA],
                            drive1_int_status.alarm_clk[A_CIA1581D1TB]);
#endif
...
static int int_cia1581d1ta(long offset)
{
    ...
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log,
                    "int_cia1581d1ta(rclk = %u, tal = %u, cra=%02x.",
	...
	if ( ( (cia1581d1ier & CIA_IM_TA) &&
	    ...
	    if(offset > cia1581d1_tal+1) {
            ...
            alarm_unset(&cia1581d1_ta_alarm); /* do _not_ clear cia1581d1_tai */
	}
    } else {
	my_unset_tai();
    }
...
#if defined(CIA1581D1_TIMER_DEBUG)
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log, "rclk=%d SDR: timer A underflow, bits=%d",
...
static int int_cia1581d1tb(long offset)
{
    ...
    if (cia1581d1_debugFlag)
	log_message(cia1581d1_log,
                    "timer B int_cia1581d1tb(rclk=%d, tbs=%d).", rclk, cia1581d1_tbs);
#endif
	...
	if (!(cia1581d1[CIA_CRB] & 8)) {
	    ...
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log,
                            "rclk=%d cia1581d1tb: set tbu alarm to %d.",
	    ...
	    if (cia1581d1_debugFlag)
		log_message(cia1581d1_log,
                            "rclk=%d cia1581d1tb: unset tbu alarm.", rclk);
#endif
	    my_unset_tbi();
	}
    } else {
...
int cia1581d1_write_snapshot_module(snapshot_t *p)
{
    ...
    log_message(cia1581d1_log, "clk=%d, cra=%02x, crb=%02x, tas=%d, tbs=%d",drive_clk[1], cia1581d1[CIA_CRA], cia1581d1[CIA_CRB],cia1581d1_tas, cia1581d1_tbs);
    log_message(cia1581d1_log, "tai=%d, tau=%d, tac=%04x, tal=%04x",cia1581d1_tai, cia1581d1_tau, cia1581d1_tac, cia1581d1_tal);
    log_message(cia1581d1_log, "tbi=%d, tbu=%d, tbc=%04x, tbl=%04x",cia1581d1_tbi, cia1581d1_tbu, cia1581d1_tbc, cia1581d1_tbl);
    log_message(cia1581d1_log, "write cia1581d1int=%02x, cia1581d1ier=%02x", cia1581d1int, cia1581d1ier);
#endif
...
int cia1581d1_read_snapshot_module(snapshot_t *p)
{
    ...
    cia1581d1_tbu = 0;
    my_unset_tbi();
    alarm_unset(&cia1581d1_tod_alarm);
...
#ifdef CIA1581D1_DUMP_DEBUG
log_message(cia1581d1_log, "read cia1581d1int=%02x, cia1581d1ier=%02x.", cia1581d1int, cia1581d1ier);
#endif
...
#ifdef CIA1581D1_DUMP_DEBUG
log_message(cia1581d1_log, "snap read rdi=%02x", byte);
log_message(cia1581d1_log, "snap setting rdi to %d (rclk=%d)", cia1581d1rdi, drive_clk[1]);
#endif
    ...
    alarm_set(&cia1581d1_tod_alarm, drive_clk[1] + dword);
