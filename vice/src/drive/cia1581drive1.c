...
static int update_cia1581d1(CLOCK rclk)
{
	...
	if (rclk != cia1581d1rdi) {
		...
		/* sets bit 7 */
		my_set_int(I_CIA1581D1FL, IK_IRQ, rclk);
	    }
	} else {
...
void REGPARM2 store_cia1581d1(ADDRESS addr, BYTE byte)
{
    ...
    if (byte != cia1581d1[addr]) {
        cia1581d1[addr] = byte;
        if (iec_info != NULL) {
            iec_info->drive_data = cia1581d1[CIA_DDRB] & ~cia1581d1[CIA_PRB];
...
BYTE read_cia1581d1_(ADDRESS addr)
{
      ...
      case CIA_ICR:		/* Interrupt Flag Register */
	{
	    ...
	    cia1581d1flag = 0;
	    cia1581d1int = 0;
	    my_set_int(I_CIA1581D1FL, 0, rclk);
...
int int_cia1581d1ta(long offset)
{
	...
	if ( ( (cia1581d1ier & CIA_IM_TA) && 
	    ...
	    if(offset > cia1581d1_tal+1) {
	        my_set_tai_clk(
			clk - (offset % (cia1581d1_tal+1)) + cia1581d1_tal + 1 );
	    } else {
	    ...
	    /* printf("cia1581d1 unset alarm: clk=%d, rclk=%d, rdi=%d -> tai=%d\n",
			clk, rclk, cia1581d1rdi, cia1581d1_tai); */
...
int int_cia1581d1tb(long offset)
{
	...
	if (!(cia1581d1[CIA_CRB] & 8)) {
		...
		if(offset > cia1581d1_tbl+1) {
		    my_set_tbi_clk( 
			clk - (offset % (cia1581d1_tbl+1)) + cia1581d1_tbl + 1);
		} else {
...
void cia1581d1_set_flag(void)
{
    ...
    if (cia1581d1[CIA_ICR] & CIA_IM_FLG) {
        my_set_int(I_CIA1581D1FL, IK_IRQ, clk);
    }
}
...
void cia1581d1_dump(FILE * fp)
{
    ...
    update_cia1581d1(clk);
    fprintf(fp, "[CIA1581D1]\n");
    fprintf(fp, "PA %d %d\n", cia1581d1[CIA_PRA], cia1581d1[CIA_DDRA]);
    fprintf(fp, "PB %d %d\n", cia1581d1[CIA_PRB], cia1581d1[CIA_DDRB]);
    fprintf(fp, "TA %u %u %d [$%02x = ", cia1581d1_tac, cia1581d1_tal,
	    cia1581d1[CIA_CRA], cia1581d1[CIA_CRA]);
    fprintf(fp, "%s%s%s%s%s]\n",
	    (cia1581d1[CIA_CRA] & 1) ? "running " : "stopped ",
	    (cia1581d1[CIA_CRA] & 8) ? "one-shot " : "continous ",
	    (cia1581d1[CIA_CRA] & 16) ? "force-load " : "",
	    (cia1581d1[CIA_CRA] & 32) ? "cnt " : "phi2 ",
	    (cia1581d1[CIA_CRA] & 64) ? "sr_out " : "sr_in ");

    fprintf(fp, "TB %u %u %d [$%02x = ", cia1581d1_tbc, cia1581d1_tbl,
	    cia1581d1[CIA_CRB], cia1581d1[CIA_CRB]);
    fprintf(fp, "%s%s%s%s]\n",
	    (cia1581d1[CIA_CRB] & 1) ? "running " : "stopped ",
	    (cia1581d1[CIA_CRB] & 8) ? "one-shot " : "continous ",
	    (cia1581d1[CIA_CRB] & 16) ? "force-load " : "",
	    (cia1581d1[CIA_CRB] & 32)
	    ? ((cia1581d1[CIA_CRB] & 64) ? "timerA+cnt" : "cnt ")
	    : ((cia1581d1[CIA_CRB] & 64) ? "timerA" : "phi2 ")
	);

    fprintf(fp, "ICR %u %u %u", cia1581d1int, cia1581d1ier,
	    drive1_int_status.pending_int[I_CIA1581D1FL]);
    fprintf(fp, " [fl= %s%s%s%s%s] ",
	    (cia1581d1int & 1) ? "TA " : "",
	    (cia1581d1int & 2) ? "TB " : "",
	    (cia1581d1int & 4) ? "Alarm " : "",
	    (cia1581d1int & 8) ? "SDR " : "",
	    (cia1581d1int & 16) ? "Flag " : "");
    fprintf(fp, "[mask= %s%s%s%s%s]\n",
	    (cia1581d1ier & 1) ? "TA " : "",
	    (cia1581d1ier & 2) ? "TB " : "",
	    (cia1581d1ier & 4) ? "Alarm " : "",
	    (cia1581d1ier & 8) ? "SDR " : "",
	    (cia1581d1ier & 16) ? "Flag " : "");

    fprintf(fp, "SR %d %d\n", cia1581d1[CIA_SDR], cia1581d1sr_bits);
    fprintf(fp, "TOD %d %d %d %d\n", cia1581d1[CIA_TOD_HR],
	    cia1581d1[CIA_TOD_MIN], cia1581d1[CIA_TOD_SEC], cia1581d1[CIA_TOD_SEC]);
}


void cia1581d1_undump_line(char *s)
{
    unsigned int d1, d2, d3, d4;

    if (s == strstr(s, "PA")) {
	sscanf(s + 2, "%u %u", &d1, &d2);
	cia1581d1[CIA_PRA] = d1;
	cia1581d1[CIA_DDRA] = d2;
	store_cia1581d1(CIA_PRA, cia1581d1[CIA_PRA]);
    } else if (s == strstr(s, "PB")) {
	sscanf(s + 2, "%u %u", &d1, &d2);
	cia1581d1[CIA_PRB] = d1;
	cia1581d1[CIA_DDRB] = d2;
	store_cia1581d1(CIA_PRB, cia1581d1[CIA_PRB]);
    } else if (s == strstr(s, "TA")) {
	sscanf(s + 2, "%u %u %u", &cia1581d1_tac, &cia1581d1_tal, &d1);
	cia1581d1[CIA_CRA] = d1;
	if ((cia1581d1[CIA_CRA] & 0x21) == 0x01) {
	    cia1581d1_tau = clk + cia1581d1_tac;
	    cia1581d1_tas = CIAT_RUNNING;
	    my_set_tai_clk(cia1581d1_tau + 1);
	} else {
	    cia1581d1_tau = 0;
	...
	sscanf(s + 2, "%u %u %u", &cia1581d1_tbc, &cia1581d1_tbl, &d1);
	cia1581d1[CIA_CRB] = d1;
	if ((cia1581d1[CIA_CRB] & 0x61) == 0x01) {
	    cia1581d1_tbu = clk + cia1581d1_tbc;
	    cia1581d1_tbs = CIAT_RUNNING;
	    my_set_tbi_clk(cia1581d1_tbu + 1);
	} else {
	    cia1581d1_tbu = 0;
	    if ((cia1581d1[CIA_CRB] & 0x61) == 0x41) {
		cia1581d1_tbs = CIAT_COUNTTA;
	    } else {
		cia1581d1_tbs = CIAT_STOPPED;
	    }
	...
	cia1581d1int = d1;
	cia1581d1ier = d2;
	if (cia1581d1int & cia1581d1ier & 0x7f) {
	    my_set_int(I_CIA1581D1FL, IK_IRQ, rclk);
	} else {
	    my_set_int(I_CIA1581D1FL, 0, rclk);
	}
    } else if (s == strstr(s, "SR")) {
	sscanf(s + 2, "%d %d", &d1, &cia1581d1sr_bits);
	cia1581d1[CIA_SDR] = d1;
    } else if (s == strstr(s, "TOD")) {
	sscanf(s + 3, "%u %u %u %u", &d1, &d2, &d3, &d4);
	cia1581d1[CIA_TOD_HR] = d1;
	cia1581d1[CIA_TOD_MIN] = d2;
	cia1581d1[CIA_TOD_SEC] = d3;
	cia1581d1[CIA_TOD_TEN] = d4;
    } else {
	printf("unknown dump format line for CIA1581D1: \n%s\n", s);
    }
}
