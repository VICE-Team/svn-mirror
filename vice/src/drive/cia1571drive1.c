...
BYTE read_cia1571d1_(ADDRESS addr)
{
    ...
    byte = 0xff;
	byte = (byte & ~cia1571d1[CIA_DDRA]) | (cia1571d1[CIA_PRA] & cia1571d1[CIA_DDRA]);
	return byte;
	break;
    ...
    byte = 0xff;
	byte = (byte & ~cia1571d1[CIA_DDRB]) | (cia1571d1[CIA_PRB] & cia1571d1[CIA_DDRB]);
        if ((cia1571d1[CIA_CRA] | cia1571d1[CIA_CRB]) & 0x02) {
	    update_cia1571d1(rclk);
	    if (cia1571d1[CIA_CRA] & 0x02) {
		byte &= 0xbf;
...
int cia1571d1_read_snapshot_module(FILE * p)
{
