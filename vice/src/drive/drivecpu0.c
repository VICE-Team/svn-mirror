...
alarm_context_t drive0_alarm_context;
...
static void reset(void)
{
    ...
    log_message(drive[0].log, "RESET.");
    cpu_int_status_init(&drive0_int_status, DRIVE_NUMOFINT, &drive0_last_opcode_info);
...
void drive0_mem_init(int type)
{
    ...
    if (type == DRIVE_TYPE_1571) {
        ...
        read_func_nowatch[0x8] = read_wd1770d0;
        store_func_nowatch[0x8] = store_wd1770d0;
    }
...
{
    alarm_context_init(&drive0_alarm_context, "Mydrive");
...
inline void drive0_cpu_wake_up(void)
{
	...
	log_message(drive[0].log, "Skipping cycles.");
	last_clk = clk;
    }
}
...
inline static int drive0_trap_handler(void)
{
    if (MOS6510_REGS_GET_PC(&drive0_cpu_regs) == 0xec9b) {
        ...
        if (drive[0].idling_method == DRIVE_IDLE_TRAP_IDLE) {
            CLOCK next_clk;
            next_clk = alarm_context_next_pending_clk(&drive0_alarm_context);
            drive_clk[0] = next_clk;
        }
        return 0;
    }
    if (MOS6510_REGS_GET_PC(&drive0_cpu_regs) == 0xc0be) {
        /* 1581 job code */
        MOS6510_REGS_SET_PC(&drive0_cpu_regs, 0xc197);
        wd1770_handle_job_code(0);
        return 0;
    }
    return 1;
}
...
#define ALARM_CONTEXT drive0_alarm_context

#define JAM() drive0_jam()

#define ROM_TRAP_ALLOWED() 1

#define ROM_TRAP_HANDLER() drive0_trap_handler()

#define CALLER e_disk8_space
...
void drive0_set_bank_base(void)
{
    JUMP(reg_pc);
}
...
int drive0_cpu_read_snapshot_module(snapshot_t *s)
{
    ...
    log_message(drive[0].log, "RESET (For undump).");
