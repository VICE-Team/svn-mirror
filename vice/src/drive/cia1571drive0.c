...
static int int_cia1571d0ta(long offset);
static int int_cia1571d0tb(long offset);
static int int_cia1571d0tod(long offset);
...
static log_t cia1571d0_log = LOG_ERR;
...
#ifdef CIA1571D0_TIMER_DEBUG
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "update: rclk=%d, tas=%d, tau=%d, tal=%u, ",
...
#ifdef CIA1571D0_TIMER_DEBUG
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "aic=%d, tac-> %u, tau-> %d tmp=%u",
                    added_int_clk, cia1571d0_tac, cia1571d0_tau, tmp);
#endif
...
#ifdef CIA1571D0_TIMER_DEBUG
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "tbc-> %u, tbu-> %d, int %02x ->",
...
#ifdef CIA1571D0_TIMER_DEBUG
		if (cia1571d0_debugFlag)
		    log_message(cia1571d0_log, "TA Reading ICR at rclk=%d prevented IRQ.",
			   rclk);
#endif
	    } else {
		if (cia1571d0ier & cia1571d0int & 0x7f) {
...
#ifdef CIA1571D0_TIMER_DEBUG
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "%02x.", cia1571d0int);
#endif
...
void reset_cia1571d0(void)
{
    ...
    if (cia1571d0_log == LOG_ERR)
        cia1571d0_log = log_open("CIA1571D0");
...
void REGPARM2 store_cia1571d0(ADDRESS addr, BYTE byte)
{
...
#ifdef CIA1571D0_TIMER_DEBUG
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "store cia1571d0[%02x] %02x @ clk=%d",
    ...
    iec_fast_drive_write(cia1571d0[CIA_SDR]);
		...
		if(cia1571d0sr_bits < 16) {
	        ...
	        if (cia1571d0_debugFlag)
	    	    log_message(cia1571d0_log, "start SDR rclk=%d.", rclk);
#endif
  	    }
	}
	break;
...
#if defined (CIA1571D0_TIMER_DEBUG)
	if (cia1571d0_debugFlag)
	    log_message(cia1571d0_log, "CIA1571D0 set CIA_ICR: 0x%x.", byte);
#endif
...
#if defined(CIA1571D0_TIMER_DEBUG)
	if (cia1571d0_debugFlag)
	    log_message(cia1571d0_log, "    set icr: ifr & ier & 0x7f -> %02x, int=%02x.",
...
#if defined (CIA1571D0_TIMER_DEBUG)
	if (cia1571d0_debugFlag)
	    log_message(cia1571d0_log, "CIA1571D0 set CIA_CRA: 0x%x (clk=%d, pc=, tal=%u, tac=%u).",
...
#if defined (CIA1571D0_TIMER_DEBUG)
	if (cia1571d0_debugFlag)
	    log_message(cia1571d0_log, "    -> tas=%d, tau=%d.", cia1571d0_tas, cia1571d0_tau);
#endif
...
#if defined (CIA1571D0_TIMER_DEBUG)
	if (cia1571d0_debugFlag)
	    log_message(cia1571d0_log, "CIA1571D0 set CIA_CRB: 0x%x (clk=%d, pc=, tbl=%u, tbc=%u).",
...
#if defined(CIA1571D0_TIMER_DEBUG)
		if (cia1571d0_debugFlag)
		    log_message(cia1571d0_log, "rclk=%d force load: set tbu alarm to %d.", rclk, cia1571d0_tbu);
#endif
		my_set_tbi_clk(cia1571d0_tbu + 1);
	    }
	}
...
#if defined(CIA1571D0_TIMER_DEBUG)
		if (cia1571d0_debugFlag)
		    log_message(cia1571d0_log, "rclk=%d start timer: set tbu alarm to %d.", rclk, cia1571d0_tbu);
#endif
		my_set_tbi_clk(cia1571d0_tbu + 1);
...
#if defined(CIA1571D0_TIMER_DEBUG)
		if (cia1571d0_debugFlag)
		    log_message(cia1571d0_log, "rclk=%d stop timer: set tbu alarm.", rclk);
#endif
		my_unset_tbi();
		cia1571d0_tbu = 0;
		if (!(byte & 0x10)) {
...
BYTE REGPARM1 read_cia1571d0(ADDRESS addr)
{
    ...
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log, "read cia1571d0[%x] returns %02x @ clk=%d.",
...
#ifdef CIA1571D0_TIMER_DEBUG
	    if (cia1571d0_debugFlag)
		log_message(cia1571d0_log,
                            "read intfl gives cia1571d0int=%02x -> %02x "
...
BYTE REGPARM1 peek_cia1571d0(ADDRESS addr)
{
      ...
      case CIA_ICR:		/* Interrupt Flag Register */
	{
	    ...
	    if (cia1571d0_debugFlag)
		log_message(cia1571d0_log,
                            "CIA1571D0 read intfl: rclk=%d, alarm_ta=%d, alarm_tb=%d.",
                            rclk, drive0_int_status.alarm_clk[A_CIA1571D0TA],
                            drive0_int_status.alarm_clk[A_CIA1571D0TB]);
#endif
...
static int int_cia1571d0ta(long offset)
{
    ...
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log,
                    "int_cia1571d0ta(rclk = %u, tal = %u, cra=%02x.",
...
#if defined(CIA1571D0_TIMER_DEBUG)
	    if (cia1571d0_debugFlag)
		log_message(cia1571d0_log, "rclk=%d SDR: timer A underflow, bits=%d",
...
static int int_cia1571d0tb(long offset)
{
    ...
    if (cia1571d0_debugFlag)
	log_message(cia1571d0_log,
                    "timer B int_cia1571d0tb(rclk=%d, tbs=%d).", rclk, cia1571d0_tbs);
#endif
	...
	if (!(cia1571d0[CIA_CRB] & 8)) {
	    ...
	    if (cia1571d0_debugFlag)
		log_message(cia1571d0_log,
                            "rclk=%d cia1571d0tb: set tbu alarm to %d.",
	    ...
	    if (cia1571d0_debugFlag)
		log_message(cia1571d0_log,
                            "rclk=%d cia1571d0tb: unset tbu alarm.", rclk);
#endif
	    my_unset_tbi();
	}
    } else {
...
int cia1571d0_write_snapshot_module(snapshot_t *p)
{
    ...
    log_message(cia1571d0_log, "clk=%d, cra=%02x, crb=%02x, tas=%d, tbs=%d",drive_clk[0], cia1571d0[CIA_CRA], cia1571d0[CIA_CRB],cia1571d0_tas, cia1571d0_tbs);
    log_message(cia1571d0_log, "tai=%d, tau=%d, tac=%04x, tal=%04x",cia1571d0_tai, cia1571d0_tau, cia1571d0_tac, cia1571d0_tal);
    log_message(cia1571d0_log, "tbi=%d, tbu=%d, tbc=%04x, tbl=%04x",cia1571d0_tbi, cia1571d0_tbu, cia1571d0_tbc, cia1571d0_tbl);
    log_message(cia1571d0_log, "write cia1571d0int=%02x, cia1571d0ier=%02x", cia1571d0int, cia1571d0ier);
#endif
...
#ifdef CIA1571D0_DUMP_DEBUG
log_message(cia1571d0_log, "read cia1571d0int=%02x, cia1571d0ier=%02x.", cia1571d0int, cia1571d0ier);
#endif
...
#ifdef CIA1571D0_DUMP_DEBUG
log_message(cia1571d0_log, "snap read rdi=%02x", byte);
log_message(cia1571d0_log, "snap setting rdi to %d (rclk=%d)", cia1571d0rdi, drive_clk[0]);
#endif
