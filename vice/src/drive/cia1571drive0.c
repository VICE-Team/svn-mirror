...
inline static void check_cia1571d0todalarm(CLOCK rclk)
{
	...
	if (cia1571d0[CIA_ICR] & CIA_IM_TOD) {
	    my_set_int(I_CIA1571D0TOD, IK_IRQ, rclk);
	}
    }
}
...
BYTE read_cia1571d0_(ADDRESS addr)
{
    ...
    byte = 0xff;
	byte = (byte & ~cia1571d0[CIA_DDRA]) | (cia1571d0[CIA_PRA] & cia1571d0[CIA_DDRA]);
	return byte;
	break;
    ...
    byte = 0xff;
	byte = (byte & ~cia1571d0[CIA_DDRB]) | (cia1571d0[CIA_PRB] & cia1571d0[CIA_DDRB]);
        if ((cia1571d0[CIA_CRA] | cia1571d0[CIA_CRB]) & 0x02) {
	    update_cia1571d0(rclk);
	    if (cia1571d0[CIA_CRA] & 0x02) {
		byte &= 0xbf;
...
int cia1571d0_write_snapshot_module(FILE * p)
{
    int byte;

    snapshot_write_module_header(p, "CIA1571D0",
			CIA_DUMP_VER_MAJOR, CIA_DUMP_VER_MINOR);

    update_cia1571d0(drive_clk[0]);

    snapshot_write_byte(p, cia1571d0[CIA_PRA]);
    snapshot_write_byte(p, cia1571d0[CIA_PRB]);
    snapshot_write_byte(p, cia1571d0[CIA_DDRA]);
    snapshot_write_byte(p, cia1571d0[CIA_DDRB]);
    snapshot_write_word(p, cia1571d0_tac);
    snapshot_write_word(p, cia1571d0_tbc);
    snapshot_write_byte(p, cia1571d0[CIA_TOD_TEN]);
    snapshot_write_byte(p, cia1571d0[CIA_TOD_SEC]);
    snapshot_write_byte(p, cia1571d0[CIA_TOD_MIN]);
    snapshot_write_byte(p, cia1571d0[CIA_TOD_HR]);
    snapshot_write_byte(p, cia1571d0[CIA_SDR]);
    snapshot_write_byte(p, cia1571d0[CIA_ICR]);
    snapshot_write_byte(p, cia1571d0[CIA_CRA]);
    snapshot_write_byte(p, cia1571d0[CIA_CRB]);

    snapshot_write_word(p, cia1571d0_tal);
    snapshot_write_word(p, cia1571d0_tbl);
    snapshot_write_byte(p, peek_cia1571d0(CIA_ICR));
    snapshot_write_byte(p, (cia1571d0_tat ? 0x40 : 0) | (cia1571d0_tbt ? 0x80 : 0));
    snapshot_write_byte(p, cia1571d0sr_bits);
    snapshot_write_byte(p, cia1571d0todalarm[0]);
    snapshot_write_byte(p, cia1571d0todalarm[1]);
    snapshot_write_byte(p, cia1571d0todalarm[2]);
    snapshot_write_byte(p, cia1571d0todalarm[3]);

    byte = cia1571d0rdi ? drive_clk[0] - cia1571d0rdi : 0;
    if(byte > 128 || byte < -16) byte = 0;
    snapshot_write_byte(p, byte);

    snapshot_write_byte(p,
		(cia1571d0todlatched ? 1 : 0) | (cia1571d0todstopped ? 2 : 0));
    snapshot_write_byte(p, cia1571d0todlatch[0]);
    snapshot_write_byte(p, cia1571d0todlatch[1]);
    snapshot_write_byte(p, cia1571d0todlatch[2]);
    snapshot_write_byte(p, cia1571d0todlatch[3]);

    snapshot_write_dword(p, drive0_int_status.alarm_clk[A_CIA1571D0TOD] - drive_clk[0]);

    return 0;
}

int cia1571d0_read_snapshot_module(FILE * p)
{
    ...
    if(strcmp(name, "CIA1571D0") || vmajor != CIA_DUMP_VER_MAJOR) return -1;
    ...
    snapshot_read_word(p, &word);
    cia1571d0_tbc = word;
    snapshot_read_byte(p, &cia1571d0[CIA_TOD_TEN]);
    snapshot_read_byte(p, &cia1571d0[CIA_TOD_SEC]);
    snapshot_read_byte(p, &cia1571d0[CIA_TOD_MIN]);
    snapshot_read_byte(p, &cia1571d0[CIA_TOD_HR]);
    snapshot_read_byte(p, &cia1571d0[CIA_SDR]);
    {

    iec_fast_drive_write(cia1571d0[CIA_SDR]);
    }
    snapshot_read_byte(p, &cia1571d0[CIA_ICR]);
    snapshot_read_byte(p, &cia1571d0[CIA_CRA]);
    snapshot_read_byte(p, &cia1571d0[CIA_CRB]);
    ...
    snapshot_read_byte(p, &cia1571d0todalarm[0]);
    snapshot_read_byte(p, &cia1571d0todalarm[1]);
    snapshot_read_byte(p, &cia1571d0todalarm[2]);
    snapshot_read_byte(p, &cia1571d0todalarm[3]);

    snapshot_read_byte(p, &byte);
    if(byte) {
	cia1571d0rdi = drive_clk[0] + byte;
    }

    snapshot_read_byte(p, &byte);
    cia1571d0todlatched = byte & 1;
    cia1571d0todstopped = byte & 2;
    snapshot_read_byte(p, &cia1571d0todlatch[0]);
    snapshot_read_byte(p, &cia1571d0todlatch[1]);
    snapshot_read_byte(p, &cia1571d0todlatch[2]);
    snapshot_read_byte(p, &cia1571d0todlatch[3]);

    snapshot_read_dword(p, &dword);
    drive0_set_alarm(A_CIA1571D0TOD, dword);

    cia1571d0_tau = drive_clk[0] + cia1571d0_tac;
    my_set_tai_clk(cia1571d0_tau + 1);
    cia1571d0_tbu = drive_clk[0] + cia1571d0_tbc;
    my_set_tbi_clk(cia1571d0_tbu + 1);

    cia1571d0_tas = (cia1571d0[CIA_CRA] & 1) ? CIAT_RUNNING : CIAT_STOPPED;
    cia1571d0_tbs = (cia1571d0[CIA_CRB] & 1) ? CIAT_RUNNING : CIAT_STOPPED;
    if((cia1571d0[CIA_CRB] & 0x41) == 0x41) cia1571d0_tbs = CIAT_COUNTTA;
