...
int int_cia1571d0ta(long offset)
{
	...
	if ( ( (cia1571d0ier & CIA_IM_TA) && 
	    ...
	    if(offset > cia1571d0_tal+1) {
	        my_set_tai_clk(
			clk - (offset % (cia1571d0_tal+1)) + cia1571d0_tal + 1 );
	    } else {
	    ...
	    /* printf("cia1571d0 unset alarm: clk=%d, rclk=%d, rdi=%d -> tai=%d\n",
			clk, rclk, cia1571d0rdi, cia1571d0_tai); */
...
int int_cia1571d0tb(long offset)
{
	...
	if (!(cia1571d0[CIA_CRB] & 8)) {
		...
		if(offset > cia1571d0_tbl+1) {
		    my_set_tbi_clk( 
			clk - (offset % (cia1571d0_tbl+1)) + cia1571d0_tbl + 1);
		} else {
...
void cia1571d0_set_flag(void)
{
    ...
    if (cia1571d0[CIA_ICR] & CIA_IM_FLG) {
        my_set_int(I_CIA1571D0FL, IK_IRQ, clk);
    }
}
...
void cia1571d0_dump(FILE * fp)
{
    ...
    update_cia1571d0(clk);
    fprintf(fp, "[CIA1571D0]\n");
    fprintf(fp, "PA %d %d\n", cia1571d0[CIA_PRA], cia1571d0[CIA_DDRA]);
    fprintf(fp, "PB %d %d\n", cia1571d0[CIA_PRB], cia1571d0[CIA_DDRB]);
    fprintf(fp, "TA %u %u %d [$%02x = ", cia1571d0_tac, cia1571d0_tal,
	    cia1571d0[CIA_CRA], cia1571d0[CIA_CRA]);
    fprintf(fp, "%s%s%s%s%s]\n",
	    (cia1571d0[CIA_CRA] & 1) ? "running " : "stopped ",
	    (cia1571d0[CIA_CRA] & 8) ? "one-shot " : "continous ",
	    (cia1571d0[CIA_CRA] & 16) ? "force-load " : "",
	    (cia1571d0[CIA_CRA] & 32) ? "cnt " : "phi2 ",
	    (cia1571d0[CIA_CRA] & 64) ? "sr_out " : "sr_in ");

    fprintf(fp, "TB %u %u %d [$%02x = ", cia1571d0_tbc, cia1571d0_tbl,
	    cia1571d0[CIA_CRB], cia1571d0[CIA_CRB]);
    fprintf(fp, "%s%s%s%s]\n",
	    (cia1571d0[CIA_CRB] & 1) ? "running " : "stopped ",
	    (cia1571d0[CIA_CRB] & 8) ? "one-shot " : "continous ",
	    (cia1571d0[CIA_CRB] & 16) ? "force-load " : "",
	    (cia1571d0[CIA_CRB] & 32)
	    ? ((cia1571d0[CIA_CRB] & 64) ? "timerA+cnt" : "cnt ")
	    : ((cia1571d0[CIA_CRB] & 64) ? "timerA" : "phi2 ")
	);

    fprintf(fp, "ICR %u %u %u", cia1571d0int, cia1571d0ier,
	    drive0_int_status.pending_int[I_CIA1571D0FL]);
    fprintf(fp, " [fl= %s%s%s%s%s] ",
	    (cia1571d0int & 1) ? "TA " : "",
	    (cia1571d0int & 2) ? "TB " : "",
	    (cia1571d0int & 4) ? "Alarm " : "",
	    (cia1571d0int & 8) ? "SDR " : "",
	    (cia1571d0int & 16) ? "Flag " : "");
    fprintf(fp, "[mask= %s%s%s%s%s]\n",
	    (cia1571d0ier & 1) ? "TA " : "",
	    (cia1571d0ier & 2) ? "TB " : "",
	    (cia1571d0ier & 4) ? "Alarm " : "",
	    (cia1571d0ier & 8) ? "SDR " : "",
	    (cia1571d0ier & 16) ? "Flag " : "");

    fprintf(fp, "SR %d %d\n", cia1571d0[CIA_SDR], cia1571d0sr_bits);
    fprintf(fp, "TOD %d %d %d %d\n", cia1571d0[CIA_TOD_HR],
	    cia1571d0[CIA_TOD_MIN], cia1571d0[CIA_TOD_SEC], cia1571d0[CIA_TOD_SEC]);
}


void cia1571d0_undump_line(char *s)
{
    unsigned int d1, d2, d3, d4;

    if (s == strstr(s, "PA")) {
	sscanf(s + 2, "%u %u", &d1, &d2);
	cia1571d0[CIA_PRA] = d1;
	cia1571d0[CIA_DDRA] = d2;
	store_cia1571d0(CIA_PRA, cia1571d0[CIA_PRA]);
    } else if (s == strstr(s, "PB")) {
	sscanf(s + 2, "%u %u", &d1, &d2);
	cia1571d0[CIA_PRB] = d1;
	cia1571d0[CIA_DDRB] = d2;
	store_cia1571d0(CIA_PRB, cia1571d0[CIA_PRB]);
    } else if (s == strstr(s, "TA")) {
	sscanf(s + 2, "%u %u %u", &cia1571d0_tac, &cia1571d0_tal, &d1);
	cia1571d0[CIA_CRA] = d1;
	if ((cia1571d0[CIA_CRA] & 0x21) == 0x01) {
	    cia1571d0_tau = clk + cia1571d0_tac;
	    cia1571d0_tas = CIAT_RUNNING;
	    my_set_tai_clk(cia1571d0_tau + 1);
	} else {
	    cia1571d0_tau = 0;
	...
	sscanf(s + 2, "%u %u %u", &cia1571d0_tbc, &cia1571d0_tbl, &d1);
	cia1571d0[CIA_CRB] = d1;
	if ((cia1571d0[CIA_CRB] & 0x61) == 0x01) {
	    cia1571d0_tbu = clk + cia1571d0_tbc;
	    cia1571d0_tbs = CIAT_RUNNING;
	    my_set_tbi_clk(cia1571d0_tbu + 1);
	} else {
	    cia1571d0_tbu = 0;
	    if ((cia1571d0[CIA_CRB] & 0x61) == 0x41) {
		cia1571d0_tbs = CIAT_COUNTTA;
	    } else {
		cia1571d0_tbs = CIAT_STOPPED;
	    }
	...
	cia1571d0int = d1;
	cia1571d0ier = d2;
	if (cia1571d0int & cia1571d0ier & 0x7f) {
	    my_set_int(I_CIA1571D0FL, IK_IRQ, rclk);
	} else {
	    my_set_int(I_CIA1571D0FL, 0, rclk);
	}
    } else if (s == strstr(s, "SR")) {
	sscanf(s + 2, "%d %d", &d1, &cia1571d0sr_bits);
	cia1571d0[CIA_SDR] = d1;
    } else if (s == strstr(s, "TOD")) {
	sscanf(s + 3, "%u %u %u %u", &d1, &d2, &d3, &d4);
	cia1571d0[CIA_TOD_HR] = d1;
	cia1571d0[CIA_TOD_MIN] = d2;
	cia1571d0[CIA_TOD_SEC] = d3;
	cia1571d0[CIA_TOD_TEN] = d4;
    } else {
	printf("unknown dump format line for CIA1571D0: \n%s\n", s);
    }
}
