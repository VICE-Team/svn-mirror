...
#define	cia1_restore_int(c)	     tpi1_restore_int(2,(c))
#define	cycles_per_sec		     machine_get_cycles_per_second()

#define	CIA1_USE_INLINE 1
...
#ifndef cia1_set_int_clk
#define	cia1_set_int_clk(value,num,clk)					\
...
static inline void my_set_int(int value, CLOCK rclk)
{
    ...
    if ((value)) {
        cia1int |= 0x80;
        cia1_set_int_clk((I_CIA1FL), (IK_IRQ), (rclk));
    } else {
        cia1_set_int_clk((I_CIA1FL), 0, (rclk));
    }
}
...
#define	my_set_int(value, rclk)						\
    do {								\
	    ...
	    fprintf(logfile, "set_int(rclk=%d, int=%d, d=%d pc=)\n",		\
		   rclk,(int_num),(value));				\
	cia1_set_int_clk((I_CIA1FL), (value), (rclk));		\
	if ((value))							\
	    cia1int |= 0x80;						\
    } while(0)
#else /* CIA1_TIMER_DEBUG */
#define	my_set_int(value, rclk)						 \
    do {								 \
        cia1_set_int_clk((I_CIA1FL), (value), (rclk));		 \
	if ((value))							 \
...
#endif /* CIA1_TIMER_DEBUG */

#endif /* CIA1_USE_INLINE */
...
BYTE read_cia1_(ADDRESS addr)
{
    ...
    byte = parallel_bus;
    if(parallel_debug) {
        fprintf(logfile, "read: parallel_bus=%02x, pra=%02x, ddra=%02x -> %02x\n",
